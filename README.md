...
| `grain completeness <file…>` | the other files this repo's own commits show changing together with the ones given more often than they change anyway (`co-changed in N/M commits`, M being the given file's own commits). The exact line the post-edit hook also appends, unprompted, whenever an edit has one. |
