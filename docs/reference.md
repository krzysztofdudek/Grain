...
| `selftest --cochange` | `--runs N`, `--json` | the co-change partners measured prospectively: learned from the oldest 80% of the retained commits, then for every file of each newer commit of 2 to 40 files, the (at most three) partners named are compared with the files that commit really touched — hit@3, the same over companions outside the 10 hottest files (non-obvious), precision@1 and how often anything is named, beside the null that names the 3 hottest files; then how many partners the same rule names over the whole history and on N swap-randomised copies of it (every one false by construction). Meant for measuring the engine |
