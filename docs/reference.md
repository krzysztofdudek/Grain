...
| `selftest --null` | `--runs N`, `--seed N`, `--json` | each family of claims (role conventions, architecture norms, birth obligations, co-change partners, commit archetypes, the language bridge, value norms, deviation fix rates) re-run on a randomisation that keeps its marginals and destroys the link it claims — role labels and directory placements dealt out again within each scope kind and the assigned scopes' predicates across partitions, out-edge sets dealt out among the files that have one, the commit × file matrix swap-randomised (50 curveball trades per commit), message tokens dealt out among commits, each value container's members given to as many declaring files as carried them at random, fix flags dealt out among all modification events — reporting what each still certifies (false by construction) beside the real count. Costs two learn passes per run; meant for measuring the engine, not for everyday use |
| `selftest --cochange` | `--runs N`, `--seed N`, `--json` | the co-change partners measured prospectively: learned from the oldest 80% of the retained commits, then for every file of each newer commit of 2 to 40 files, the (at most three) partners named are compared with the files that commit really touched — hit@3, the same over companions outside the 10 hottest files (non-obvious), precision@1 and how often anything is named, beside the null that names the 3 hottest files and beside the same rule with a base rate per commit instead of per commit size; then how many partners each rule names over the whole history and on N swap-randomised copies of it (every one false by construction); `--seed` picks the first run's random source, each further run the next. Meant for measuring the engine |
...
| --- | --- | --- | --- |
| `cochange` | `co-change: <file> (co-changed in k/N commits)` | a file this repo's history changes together with the ones given, more often than it changes anyway (the co-change cell, §mathematics "Co-change partners"), is not in the change | `check <file>`, `review`, `how` (`completeness` reads the identical `cochangeData` through its own dedicated renderer, `completenessDirectional`, never through `missingLines`) |
...
- **PostToolUse on Edit, Write and MultiEdit** runs `grain check-hook`: the edited file is re-checked and grain
  ...
  co-changing with the one you just touched (`cochangeData`, the co-change cell) — this line fires even when
...
`partition` (issue 263) reads each graph as a partition of the files it owns, a file belonging to the deepest node whose mapping selects it, over the files both graphs own. `leaves` compares the two leaf partitions: `hPgivenA` is H(P|A), the bits still needed to name a file's proposed node once its accepted node is known (how much finer the proposal cuts), `hAgivenP` is H(A|P) (how much finer the accepted graph cuts), `vi` their sum, with `nmi` (mutual information over the geometric mean of the two entropies), `ari`, `purity` and `inversePurity`. `byDepth` repeats the comparison with the accepted tree cut at each depth (an accepted node stands for its descendants), and `lowestVI` names the depth whose VI is lowest: the level of the accepted tree the proposal cuts at. Near zero in one direction and large in the other means the two graphs differ in granularity, not in where they cut. `relationsProjected` maps each accepted node to the proposed node owning most of its files (its subtree's, when it maps none itself) and scores every accepted relation in the proposal's own node names: `acceptedPairs` distinct projected pairs, `acceptedCollapsed` relations whose two ends land in one proposed node, `acceptedUnmappable` those with an end that owns no file, and `overlap`, `recall` and `precision` against the proposal's own relations. Both blocks are additive; the Jaccard measures are unchanged.
