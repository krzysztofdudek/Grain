...
A kind with one group needs a reference of its own (issue 390). Its group is every scope of that kind induction assigned in the partition, so against the partition it still wins by being assigned, the confound above, and relabelling inside the partition cannot move it. It is contrasted instead with the scopes of its kind that induction assigned in every other partition: assigned against assigned, as with several groups, and an outside population, as for a partition-wide absence. `learn()` sums the assigned scopes' weighted outcomes over the repository, an ambiguous member at half, and `mine()` subtracts its own partition's. A boolean predicate that another partition's vocabulary does not carry is not done there, so those scopes count as `false`; without that, a call only this partition makes had no reference at all. A placement predicate is left out: partitions are cut on directories, so "lives under `handlers/`" would win against every other partition by construction. With no scope of the kind assigned anywhere else, the cell has nothing to be contrasted with and says nothing. The label null deals the assigned scopes' predicates out again across partitions as well, so a lone group is tested too.
...
| ½ | learn.mjs | `baseW(s) * (ri.amb.has(i) ? 0.5` | derived — the ambiguous member's half vote, in the repository-wide pool of assigned scopes a one-group role cell is contrasted with | — |
