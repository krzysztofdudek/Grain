...
// ==================================================================================================
// 7a. `provenance.json` — one per rendered aspect (law-loop-yggdrasil.md §1.2).
//
...
//
// `status`, `draftReason` and `scopeApproximation` are ADDED here, not shared with law-loop.mjs's
...
export function provenanceFor(a, { asOf, repo }) {
  return {
    ...
    // direction whose sentence a reader must not read as a prohibition; null otherwise.
    ...
    // asking: how much of the code that is already here does this rule refuse on day one? For an
    ...
    // 'enforced' | 'advisory' | 'draft' — the SAME three values Yggdrasil's own `yg-aspect.yaml`
    ...
    // it is the one reason that says the row was never eligible to earn a status at all.
...
// WHAT THE STATUS DOES, IN ONE SENTENCE, BESIDE THE RULE. `status: advisory` is a word whose
...
// ==================================================================================================
// 7a-continued. Aspect status, earned rather than declared (sharpened by the ruling
// `enforced-requires-certified-origin`). Sits right after `provenanceFor` (§7a) rather than claiming its own
// top-level number — §7b below (the family-candidates adapter) keeps its own number, unrenumbered.
//
...
//     Exactly what this renderer shipped before the status became earned — the absence of a verdict is not one of the three
