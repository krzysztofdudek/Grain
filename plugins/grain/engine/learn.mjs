...
// the randomisations `grain selftest --null` asks learn() for live in null-shuffles.mjs; re-exported for callers
export { shuffleAssignedAcrossPartitions, shuffleFixes, shuffleLabels, shuffleMembers } from './null-shuffles.mjs';
import { shuffleAssignedAcrossPartitions, shuffleFixes, shuffleLabels, shuffleMembers } from './null-shuffles.mjs';
...
export async function learn({
  ...
  // the same null for a kind with ONE role group in its partition, which the label shuffle cannot move (issue 390):
  // such a group is contrasted with the scopes of its kind assigned in the other partitions, so the link it claims
  // is the partition, and it is destroyed by dealing the assigned scopes' predicates out again across partitions
  if (nullLabels) shuffleAssignedAcrossPartitions(prepared, nullLabels);
  for (const pr of prepared) Crepo += countCandidates(pr.ps, pr.ri);
  // every assigned scope's weighted outcomes per (kind, predicate) over the whole repository, weighted as mine()
  // weighs a role cell (an ambiguous member at half): a one-group kind's role cell is contrasted with this, less its
  // own partition's share (mine(), issue 390)
  // `kind` alone (no predicate) keys the weight of every assigned scope of the kind: a scope elsewhere whose partition
  // has no such boolean predicate in its vocabulary does not do the thing there, and counts as `false`
  const repoRole = new Map();
  for (const { ps, ri } of prepared)
    for (const [i, r] of ri.assign) {
      if (r === undefined) continue;
      const s = ps[i],
        w = baseW(s) * (ri.amb.has(i) ? 0.5 : 1);
      if (Object.keys(s.preds).length) repoRole.set(s.kind, (repoRole.get(s.kind) || 0) + w);
      for (const [pid, v] of Object.entries(s.preds)) {
        const k = s.kind + S + pid;
        const t = repoRole.get(k) || repoRole.set(k, Object.create(null)).get(k);
        t[v] = (t[v] || 0) + w;
      }
    }
