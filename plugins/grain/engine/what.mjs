...
export function whatCmd({
  ...
  // query was enough to claim an unrelated symbol as a hit, with full confidence. The query's OWN tokens
  ...
  for (const c of cards) {
    ...
    if (c.type === 'file') {
      ...
      for (const k of c.members || []) {
        ...
        // a catch/finally member's `name` is its enclosing method/type's OWN name (blockScope's borrowed
  ...
  // an exact-name match sorts first, ahead of the old rel/line order — a display cap must show the true
  ...
  // exactLocal — is the query the EXACT name of something declared here (case-insensitive), not merely
  ...
  //
  // Computed over the FULL sorted set, BEFORE the splice(12) display cap two lines down. With heavy
  ...
  // the same lesson, one step further: everything DERIVED from the declaration hits is computed
  ...
  const spreadFiles = new Set(definedAll.map(d => d.rel)); // the full set, never the capped one
  ...
  // (d) siblings — DELETED. It printed the OTHER members of any container a matched value sits in, which
  // by construction is exactly the set of values that did NOT match the query. Measured across 7 languages
  // (the maintainer notes' measurement log): per-value precision 0.364 [0.29–0.44] over 165 blind hand
  ...
  // PUSH surface — volunteered inside the answer to a different question — and the ruling that retired twin rows is that a push
  ...
  //
  // The evidence keeps its home, exactly as that ruling kept `model.twins`: `model.valueSiblings`/`valueContainer`/
  ...
  // a bare count could not be acted on: a reader had to fall back to grep to find the actual files, making
  ...
  // rendered list capped the same way `defined`'s own display cap works two screens up (a cap decides only
  // what is SHOWN, never what is measured — `total` always carries the true count, uncapped).
  let usedBy = null;
  let top3 = new Set();
  if (definedAll.length) {
    const byFile = new Map();
    for (const d of definedAll) byFile.set(d.rel, (byFile.get(d.rel) || 0) + 1); // ranked over every hit, not the twelve shown
    top3 = new Set(
      [...byFile]
        .sort((a, b) => b[1] - a[1] || (a[0] < b[0] ? -1 : 1))
        .slice(0, 3)
  ...
  // (f2) tested by: same-stem naming, else co-change/import evidence — see testedByEvidence's own note.
  ...
  // (g) structural references — see `typeRefHits`'s own note. Only consulted for a name with no exact
  ...
  let note = null;
  // the disclosure register, same convention as whereCmd's own: every hedge below is ALSO recorded here as
  ...
  // grow a sibling.
  const disclosures = [];
  // until now every honest-negative disclosure fired ONLY on an empty answer, and the field showed that is
  ...
  // and no value carries the name, they only share words with it. That predicate is only trustworthy because the full-set computation
  // computes `exactLocal` over the full set, before the display cap.
  //
  // The ≥2-token condition is not a tuning knob; it is the same two-token cut applied to the same evidence. For a SINGLE-
  ...
  // overconfident by a hedge, so it keeps the looser substring scan over every blind file. Different claims,
  ...
  if (!defined.length && !valueHits.length && !referenced) {
    // no INDEXED presence — but "indexed" and "exists" are not the same claim: a gated
    // value (seen, excluded by the df floor) or a symbol whose exact text lives in a zero-scope file (the Rust-macro-body and Go-package-const
    ...
    if (gv) {
      ...
      // a same-container sibling list, when the gated evidence found one (see gatedValueEvidence's own
      ...
      // a certified-absence sibling stronger than `blindHit` below: the exact text was found, on a
  ...
  // `definedTotal`/`spreadFiles`/`weakName` are internal: `cmdWhat` destructures the published fields
