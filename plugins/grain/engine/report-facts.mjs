...
// (the dirty-tree ruling) `+dirty` is spoken for: it means "this answer incorporates your uncommitted edits", true
// only of `check`/`review` (always) and `spectrum`/`explain` (for the one file asked about, since the dirty-tree ruling made them
...
// a lexical style surface is a per-FILE majority vote (lexicalPreds) — `auto.lex:quote` reads `double` while at
...
// (a follow-up to the quote-style disclosure): of the minority-quote literals a per-file quote vote counts
// as departing (lexTally's `off`, below), a delimiter forced by the literal's OWN body (`'he said "hi"'` in a
// double-quote file — the other quote would need escaping) is not a style choice; the quote-style disclosure already measured this
...
// field and the printed clause can never disagree about the same file. `flags` is the subset of the hidden
// instances that are genuine violations, not delimiter-forced (quoteFlags above) — always [] for the three
// non-quote lexical surfaces, so their note is byte-for-byte unchanged from the quote-style disclosure.
...
// a `report`/`rules` TEMPLATE line (mineTemplates/profileOf — unclustered residue, never a role group) is a
...
// template line (the exact drift this repo already fixed once for a different disclosure).
...
// The {n, grammars} shape is exported (not just the prose below) so export.mjs can carry the identical
// fact `report`/`status` print — one function computes it, so the two surfaces can never drift apart the way
// rules/report once did.
...
// the sibling gap: every import CAN be resolution-supported and genuinely resolved (model.edges nonempty)
...
// a "module" here is a directory bucket — moduleOf/refineModOf (relations.mjs), refined one path segment
...
// report() and rulesMarkdown() say the identical thing about the identical cycle (the drift this repo
...
export function healthRows(model, outcomes) {
  ...
  // MEASURED (3 languages, 75 rows hand-adjudicated against a criterion fixed before the pairs were seen,
