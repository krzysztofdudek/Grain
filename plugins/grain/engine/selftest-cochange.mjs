...
// a model carrying only what the co-change cell reads; without `fileTouches` the cell falls back to the base rate
// per commit, the rate before commit size was accounted for (issue 366), kept here as the comparison arm
const cellModel = (Hx, sized) => ({
  cochange: Hx.cochange,
  nonMegaCommits: Hx.nonMegaCommits,
  fileTouches: sized ? Hx.fileTouches : 0,
  pathsAll: [],
  filesAll: [],
});
const CELLS = { cell: true, perCommit: false };

// the arms: the shipped cell, the cell with a base rate per commit, and the null that names the three files the
// training window touched most
function arms(Hx) {
  const out = {};
  for (const [arm, sized] of Object.entries(CELLS)) {
    const m = cellModel(Hx, sized);
    const memo = new Map();
    out[arm] = f => {
  ...
  out.hot = Object.entries(Hx.fileCommits)
    .sort((a, b) => b[1] - a[1] || (a[0] < b[0] ? -1 : 1))
    .map(([f]) => f);
  return out;
}
// every (edited file, partner) pair each cell names over one history, dead partners included: the cell is
// measured, not the rendering
function namedPairs(Hx) {
  const A = arms(Hx);
  const files = new Set();
  for (const c of Hx.cochange) files.add(c.a).add(c.b);
  const n = {};
  for (const arm of Object.keys(CELLS)) {
    n[arm] = 0;
    for (const f of files) n[arm] += A[arm](f).length;
  }
  return n;
}
export function cochangeEval({ H, runs = 3, seed = 1 }) {
  ...
  const acc = { cell: score(), perCommit: score(), hottest: score() };
  ...
  for (const [arm, r] of Object.entries(acc))
    out.arms[arm] = {
      ...
      counts: r, // the raw counts, so runs over several repositories can be pooled
    };
  ...
  const real = fps.length ? namedPairs(aggregatesOf(fps)) : { cell: 0, perCommit: 0 };
  for (const arm of Object.keys(CELLS)) {
    const xs = nulls.map(x => x[arm]);
    Object.assign(out.arms[arm], {
      real: real[arm],
      null: xs,
