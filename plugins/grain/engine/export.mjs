...
export function featureOf(pid) {
  const m = pid.match(/^auto\.([a-z0-9]+)(?::(.*))?$/);
  if (!m) return { enumerator: pid, argument: null, family: 'other' };
  const [, e, arg] = m;
  const family =
    {
      nameshape: 'identity',
      filenameshape: 'identity',
      deco: 'identity',
      extends: 'identity',
      returns: 'identity',
      arity: 'identity',
      ...
      dir2: 'placement',
      dir3: 'placement',
      moddirshape: 'placement',
      modfileshape: 'placement',
      modsize: 'placement',
      lex: 'lexical',
    }[e] || 'other';
...
function contextOf(cid, part) {
  ...
  const r = +cid.slice(1).split(':')[0];
  return { type: 'group', group: 'r' + r, label: part.medoids[r]?.label || 'group' };
}
const inContext = (ctx, s, role) =>
  ctx.type === 'partition'
    ? true
    : ctx.type === 'directory'
      ? s.rel.startsWith(ctx.dir + '/')
      : role === +ctx.group.slice(1);
...
function focusLines(lines, s, pid, exp) {
  ...
  const from = s.line,
    to = Math.min(s.endLine || s.line, lines.length);
  const scan = (a, b, re, cap = 4) => {
    const out = [];
    for (let i = Math.max(1, a); i <= Math.min(b, lines.length) && out.length < cap; i++)
  ...
  if (enumerator === 'imp') {
    const tail = argument.split('/').pop();
    const f = scan(
      1,
      lines.length,
      new RegExp('(import|require|use|include|from|using)\\b.*\\b' + esc(tail) + '\\b'),
      6
    );
    return f.length ? f : [];
  }
  if (enumerator === 'call') {
    let f = scan(from, to, new RegExp(esc(argument) + '\\s*\\('), 6);
    if (!f.length)
      f = scan(from, to, new RegExp('(^|[^\\w.])' + esc(argument.split('.').pop()) + '\\s*\\('), 6); // `this.client\n  .send(` — the head splits across lines, its last segment does not
    return f.length ? f : [from];
  }
  if (enumerator === 'returns') {
    // the return type sits wherever the (possibly multi-line) signature puts it — measured: 29% of
    // multiline-signature sites had the type off the declaration line, and every one of them anchored to the wrong line
    const f = scan(
      from,
      Math.min(from + 12, to),
      new RegExp('(^|[^\\w])' + esc(argument.replace(/^[?!]/, '')) + '($|[^\\w])'),
      2
    );
    return f.length ? [f[0]] : [from];
  }
  ...
  if (enumerator === 'ret') {
    const f = scan(from, to, /\breturn\b/, 6);
    return f.length ? [f[f.length - 1]] : [to];
  }
...
function nearestExemplar(dev, conforming, roleOfKey) {
  ...
  for (const c of conforming) {
    let sc = 0;
    if (c.rel === dev.rel) sc = 100;
    else {
      const cs = dirname(c.rel).split('/');
      let k = 0;
      while (k < dsegs.length && k < cs.length && dsegs[k] === cs[k]) k++;
      sc = k * 2;
    }
    if (roleOfKey(c.key) !== undefined && roleOfKey(c.key) === roleOfKey(dev.key)) sc += 1;
    if (sc > bs || (sc === bs && (c.rel < best.rel || (c.rel === best.rel && c.line < best.line)))) {
      best = c;
      bs = sc;
    }
  }
  return best
    ? { rel: best.rel, line: best.line, endLine: best.endLine, name: best.name, kind: best.kind }
    : null;
}
...
export function exportModel({
  ...
  const linesOf = rel => {
    ...
    if (!fileLines.has(rel)) {
      try {
        fileLines.set(rel, readFileSync(join(root, rel), 'utf8').split(/\r?\n/));
      } catch {
        fileLines.set(rel, null);
      }
    }
  ...
  const out = {
    ...
    schemaNotes: {
      ...
      archNorms:
        'established layering per (source module, target module) pair, decided as a two-population contrast (§mathematics, "Architecture norms"): the source module\'s capable files (those with at least one resolved out-edge) against the capable files outside both modules, under the KT/BIC/index-cost test and the lambda bound every other convention uses. `exp` is the established majority ("true" = the module reaches the target, "false" = it does not), `ne`/`neff` its evidence and capable population, `share` = ne/neff, `kOut`/`nOut` how many of the capable files elsewhere reach the target — the rate the norm was contrasted with. A pair the source never crosses can be a norm (`ne === neff`, `exp: "false"`). A pair absent here cleared no acceptance floor — no claim, not "false". The in-memory model also carries finer (role-group, target module) rows (`fromKind: "group"`) — deliberately excluded here: this export\'s schema describes a (module, module) pair, and a group-level row would need a second, differently-shaped entry a consumer could not tell apart from this one without also checking `fromKind` on every row; left internal until there is a concrete consumer for it.',
      exemplars:
      ...
      moves:
        'compressed rename affinity, keyed `<file-suffix>#<name-token>`: how many times a file born under that suffix/token has historically moved from one directory to another (§mathematics, "Placement"). Lets placement advice cite a supermajority historical move without touching git history at query time.',
      valueSiblings:
        'certified value concordance (§mathematics, "Value concordance"): one entry per container (an enum, or a positionally-identified string set) whose members were found to travel together above the acceptance floor. `members` names every surviving sibling value; `norm` (present only when the co-travel itself cleared the KT/lambda test against independence of the members given their own shares; `q` is the share of complete carriers independence would give) gives the population/evidence a `kin:` gap is measured against. The raw per-value place index this is built from (`model.valueIndex` — every indexed value and everywhere it occurs, not only ones with a sibling) is NOT exported: it is internal working data on the order of `summary.valueIndexSize` entries, most of them singletons with no concordance to report; `valueSiblings`/its `norm` are the certified, bounded facts that raw index would otherwise duplicate less usefully.',
      relCoverage:
    ...
    summary: {
      files: model.files,
      partitions: model.partitions.length,
      groups: 0,
      conventions: 0,
      scopes: 0,
      deviations: 0,
      calibrationAvailable: 0,
      trendAvailable: 0,
      valueIndexSize: Object.keys(model.valueIndex || {}).length,
    },
    steers: (model.steers || []).map(st => ({ ...st })),
    ...
    twins: model.twins || [],
    moves: model.moves || {},
    valueSiblings: Object.fromEntries(
      Object.entries(model.valueSiblings || {}).map(([c, members]) => [
        c,
        { container: model.valueContainer?.[c] ?? null, members, norm: model.valueNorms?.[c] || null },
      ])
    ),
    partitions: [],
    conventions: [],
  ...
  for (const part of model.partitions) {
    ...
    const ps = scopesAll
      .filter(s => fileSet.has(s.rel) && s.kind !== 'module' && s.name !== '<anon>')
      .map(hydrateScope);
    addModuleScopes(ps);
    for (const s of ps) applyVocab(s, part.vocab);
    const keyOf = s => (s.kind === 'module' ? s.rel + '#module#' + s.name : skeyR(s.rel, s));
    const roleOf = s => {
      const r = part.assignments[keyOf(s)];
      return r === undefined || r === -1 ? undefined : r;
    };
    const roleOfKey = k => {
      const r = part.assignments[k];
      return r === undefined || r === -1 ? undefined : r;
    };
    const site = s => ({
      key: keyOf(s),
    ...
    const lifecycleOf = key => {
      ...
      return L
        ? {
            firstSeen: iso(L.first),
            lastTouched: iso(L.last),
    ...
    for (const [k, r] of Object.entries(part.assignments)) {
      if (r === -1) continue;
      (byRole.get(r) || byRole.set(r, []).get(r)).push(k);
    }
    ...
    part.medoids.forEach((md, r) => {
      ...
      for (const k of members) {
        ...
        const sh = nameShape(basename(rel, extname(rel)));
      ...
      P.groups.push({
        ...
        label: md.label,
        size: members.length,
        lift: +(part.roleLift?.[r] || 0).toFixed(2),
        implied: (part.groupImplied || {})[r] || null,
        profile: (part.profiles || {})[r] || null,
        markers: md.feats.filter(f => /^(dec|sup|ret):/.test(f)),
        nameTokens: md.feats.filter(f => f.startsWith('tok:')).map(f => f.slice(4)),
        imports: md.feats.filter(f => f.startsWith('imp:')).map(f => f.slice(4)),
        directories: [...dirs]
          .sort((a, b) => b[1] - a[1] || (a[0] < b[0] ? -1 : 1))
          .map(([dir, n]) => ({ dir, n })),
        fileNameShape: topShape
          ? {
              shape: topShape[0],
              words: shapeWords(topShape[0]),
              share: +(topShape[1] / members.length).toFixed(2),
            }
          : null,
        conventions: part.facts
          .filter(f => f.cid.startsWith('r' + r + ':'))
          .map(f => part.name + '::' + f.cid + '::' + f.pid),
        members: members.slice(0, 200).map(k => {
          const [rel, kind, name] = k.split('#');
          return { rel, kind, name, line: lineOfKey(k), endLine: endLineOfKey(k) };
        }),
      });
    });
    ...
    const localDirs = new Set(
      part.facts.filter(f => f.cid.startsWith('d[')).map(f => f.cid.slice(2, f.cid.indexOf(']')))
    );
    for (const [d, n] of [...dirScopes].sort((a, b) => (a[0] < b[0] ? -1 : 1)))
      if (n >= 8 || localDirs.has(d))
        P.directories.push({
          dir: d,
          scopes: n,
          files: (part.files || []).filter(f => f.startsWith(d + '/')).length,
          conventions: part.facts
            .filter(f => f.cid === 'd[' + d + ']:' + f.kind)
            .map(f => part.name + '::' + f.cid + '::' + f.pid),
        });
        ...
        marker:
          pre === 'deco'
            ...
            : pre === 'sup'
              ? 'extends ' + name
              : 'returns ' + name,
        type: pre === 'deco' ? 'decorator' : pre === 'sup' ? 'supertype' : 'returnType',
        name,
        carriers: keys.map(k => {
          const [rel, kind, nm] = k.split('#');
          return { rel, kind, name: nm, line: lineOfKey(k), endLine: endLineOfKey(k) };
        }),
      });
    }
    // ---- conventions with every site
    for (const f of part.facts) {
      const id = part.name + '::' + f.cid + '::' + f.pid;
      const ctx = contextOf(f.cid, part);
      const conforming = [],
        deviating = [];
      const neff = Object.values(f.counts).reduce((a, b) => a + b, 0);
      const K = isBool(f.pid) ? 2 : f.alphabet.length + 1;
      for (const s of ps) {
        if (s.kind !== f.kind || !inContext(ctx, s, roleOf(s))) continue;
        ...
        if (v === f.exp) conforming.push(st);
          ...
          const gn = Object.values(gc).reduce((a2, b2) => a2 + b2, 0);
          const known = f.alphabet.includes(v);
          const gap = Math.log2(kt(gc, K, f.exp, gn) / kt(gc, K, known ? v : UNSEEN, gn));
          deviating.push({
            ...st,
            observed: v,
            phrase: deviationPhrase(f, v),
            novel: !known,
            gapBits: +gap.toFixed(2),
            fires: gap >= (f.tau || Math.log2(CFG.lambda)),
            risingAlternative: !!(f.suppressedValue && v === f.suppressedValue),
          });
        }
      }
      const bySite = (a, b) => (a.rel < b.rel ? -1 : a.rel > b.rel ? 1 : a.line - b.line);
      conforming.sort(bySite);
      deviating.sort(bySite);
      const decorate = st => {
        const lines = linesOf(st.rel);
        const focus = focusLines(lines, st, f.pid, f.exp);
        const lc = lifecycleOf(st.key);
        const r = {
          rel: st.rel,
          kind: st.kind,
          name: st.name,
      ...
      const lifecycle = (() => {
        ...
        const lc = L(conforming),
          ld = L(deviating);
        let repairs = 0,
          departures = 0,
          lastRepair = 0,
          lastDeparture = 0;
        for (const s of conforming.concat(deviating)) {
          const evs = H.vev.get(s.key);
          if (!evs) continue;
          let prev;
          for (const e of evs) {
            ...
            if (prev !== undefined && prev !== v) {
              if (v === f.exp) {
                repairs++;
                lastRepair = Math.max(lastRepair, e.ts);
              } else if (prev === f.exp) {
                departures++;
                lastDeparture = Math.max(lastDeparture, e.ts);
              }
            }
            prev = v;
          }
        ...
        return {
          firstConforming: iso(lc.length ? Math.min(...lc.map(x => x.first)) : 0),
          lastConformingTouch: iso(lc.length ? Math.max(...lc.map(x => x.last)) : 0),
          firstDeviating: iso(ld.length ? Math.min(...ld.map(x => x.first)) : 0),
          lastDeviatingTouch: iso(ld.length ? Math.max(...ld.map(x => x.last)) : 0),
          repairs,
          departures,
          lastRepair: iso(lastRepair),
          lastDeparture: iso(lastDeparture),
          valueTracked:
            ['auto.nameshape', 'auto.first1', 'auto.ret'].includes(f.pid) ||
            /^auto\.(deco:@|extends:)/.test(f.pid),
        };
      })();
      const feat = featureOf(f.pid);
      const negated = f.exp === 'false';
      const conv = {
        id,
        partition: part.name,
        context: ctx,
        ...
        expected: f.exp,
        negated,
        packageWide: !!f.pkgWide,
        seeded: f.seeded || [],
        contested: f.contested || null,
        statement: verbalize(
          f,
          f.exemplars.map(e => e.name)
        ),
        parentDefault: f.parentExp ?? null,
        localContrast: ctx.type !== 'partition' && f.parentExp != null && f.parentExp !== f.exp,
        alphabet: f.alphabet,
        counts: f.counts,
        established: f.sraw,
        share: f.share,
        bitsPerInstance: f.bpi,
        gapThresholdBits: f.tau,
        surfaces: f.nSurfaces,
        siblings: (f.siblings || []).map(sb => ({
          pid: sb.pid,
          feature: featureOf(sb.pid),
          expected: sb.exp,
          statement: verbalize({ ...sb, kind: f.kind }, []),
        })),
        trend: f.trend
          ? {
        ...
        sites: {
          ...
          firing: deviating.filter(d => d.fires).length,
          truncated: conforming.length > maxSites ? conforming.length - maxSites : 0,
        },
        ...
        deviatingSites: deviating.map(d => ({
          ...decorate(d),
          observed: d.observed,
          phrase: d.phrase,
          novel: d.novel,
          gapBits: d.gapBits,
          fires: d.fires,
          risingAlternative: d.risingAlternative,
          nearest: nearestExemplar(d, conforming, roleOfKey),
        })),
        check: {
          scope: f.kind,
          context: ctx,
          enumerator: feat.enumerator,
          argument: feat.argument,
          expected: f.exp,
          negated,
          applicableNodeTypes:
            feat.enumerator === 'deco'
              ? part.vocab.DNT || null
              : feat.enumerator === 'extends'
                ? part.vocab.ENT || null
                : feat.enumerator === 'returns'
                  ? part.vocab.RNT || null
                  : null,
          reference: 'grain check --json <file> evaluates this rule on a worktree file',
        },
      };
      out.conventions.push(conv);
      P.conventions.push(id);
    ...
    out.summary.groups += P.groups.length;
    out.summary.scopes += part.scopes;
    out.partitions.push(P);
  }
  out.summary.conventions = out.conventions.length;
  out.cochange = (model.cochange || [])
    .slice(0, 2000)
    .map(p => ({
      a: p.a,
      b: p.b,
      support: p.sup,
      commitsA: p.commitsA ?? null,
      commitsB: p.commitsB ?? null,
      confidenceAB: p.commitsA ? +(p.sup / p.commitsA).toFixed(2) : (p.conf ?? null),
      confidenceBA: p.commitsB ? +(p.sup / p.commitsB).toFixed(2) : null,
    }));
  return out;
}
