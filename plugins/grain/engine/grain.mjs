...
import { hydrateScope, applyVocab, partitionFor, verbalize as verb2, baselineShare, DIRTY_TREE_NOTE, heritageKindOf } from './core.mjs';
...
export function parseArgv(argv) {
  ...
  for (let i = 0; i < argv.length; i++) { const a = argv[i];
    ...
    if (a.startsWith('--')) { const [k, ...v] = a.slice(2).split('='); if (v.length) opts[k] = v.join('='); else if (['repo', 'top', 'minbits', 'as', 'content', 'mode', 'map-rows', 'out', 'max-sites', 'surfaces', 'instead-of', 'never-imports', 'weight', 'topic', 'note', 'author', 'range', 'on', 'last'].includes(k) && argv[i + 1] !== undefined && !argv[i + 1].startsWith('--')) opts[k] = argv[++i]; else opts[k] = true; }
    else args.push(a); }
...
export async function ensureFresh({ root, isGit, store, opts, want = 'refresh' }) {
  ...
  const extractOk = meta && meta.engine === ENGINE_VERSION && meta.extractor === EXTR_V && meta.grammars === grammarStamp();
  const versionOk = extractOk && (meta.model || '') === MODEL_V;
  const { seeds, boundaries, waivers } = readSeeds(store); const seedsHash = hashSeeds({ seeds, boundaries, waivers });
  ...
  const tree = isGit && head ? headTree(root, { skip: (rel, sha) => !!(treeCache && treeCache[sha + '|' + rel]) }) : null;
  const { model: m2, ms, scopes, rawScopes, treeCacheOut } = await learn({ root, H, log, tree, treeCache, seeds, boundaries, waivers });
...
function repoDirty(root, isGit) { if (!isGit) return false;
  let out; try { out = execFileSync('git', ['-C', root, 'status', '--porcelain'], { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] }); } catch { return false; }
  return out.split('\n').some(line => { const p = toPosix(line.slice(3)); return p && !HARD_EXCL.test(p); }); }
...
export async function cmdHow({ model, root, isGit, args, opts, stamp, store, treeDirty }) {
  ...
  let H = null; if (isGit && !opts['no-history']) { try { H = (await loadHistory({ gitdir: root, store, log })).H; } catch (e) { log('history unavailable for how: ' + e.message); } }
  if (!H || !H.fps || !H.fps.length) { // no git, --no-history, a shallow clone, or an unreadable history: say so and stop, never crash
    const why = !isGit ? 'this is not a git repository' : opts['no-history'] ? '--no-history was passed' : 'this repository has no readable commit history';
    ...
    if (opts.json) return [JSON.stringify({ query, matches: [], places: [], missing: [], shape: null, note, asOf: stamp().replace(/^as of /, '') })];
  ...
  const msgOf = sha => { if (msgCache.has(sha)) return msgCache.get(sha);
    let m = null; try { m = execFileSync('git', ['-C', root, 'show', '-s', '--format=%s', sha], { encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] }).trim(); } catch { /* a sha git can no longer resolve: fall back to the indexed tokens */ }
    msgCache.set(sha, m); return m; };
  const { lines, matches, places, missing, shape } = howCmd({ model, H, query, top: +opts.top || 5, msgOf, mapRows: +opts['map-rows'] || 60, exemplarOk: existsMemo(root) });
  if (opts.json) return [JSON.stringify({ query, matches: matches.map(m => ({ sha: m.sha, ts: m.ts, msg: m.msg, files: m.files, score: m.score })), places, missing, shape, asOf: stamp().replace(/^as of /, '') })];
  return [...lines, ...(treeDirty ? [DIRTY_TREE_NOTE] : []), stamp()]; }
...
function findBlindHit(model, root, query, strict = false) {
  if (!strict) { // §018's original path, byte-for-byte: first blind file whose raw text contains the query
    ...
    return null; }
  const re = new RegExp('(^|[^A-Za-z0-9_$])' + query.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') + '([^A-Za-z0-9_$]|$)', 'g');
...
export async function cmdWhat({ model, root, isGit, args, opts, stamp, store, treeDirty }) {
  ...
  const query = args.join(' ');
  let H = null; if (isGit && !opts['no-history']) { try { H = (await loadHistory({ gitdir: root, store, log })).H; } catch (e) { log('history unavailable for what: ' + e.message); } }
  ...
  if (opts.json) return [JSON.stringify({ query, defined, values, spread, siblings, changes, usedBy, referenced: referenced || null, note: note && note.kind !== 'absent' ? note : null, asOf: stamp().replace(/^as of /, '') })];
  return [...lines, ...(treeDirty ? [DIRTY_TREE_NOTE] : []), stamp()]; }
...
export async function cmdMap({ model, args, opts, stamp, treeDirty }) {
  if (args.length) throw new Error('usage: grain map [--json] — takes no file argument; for one file, use `grain explain <file>` (or `spectrum`)');
  if (opts.json) return [JSON.stringify({ nodes: (model.moduleGraph?.nodes || []).map(n => ({ id: n.id, layer: n.layer })), decisions: (model.steers || []).length + (model.boundaries || []).length + (model.waivers || []).length, asOf: stamp().replace(/^as of /, '') })];
...
function govFactsOf(r, touched) { const m = new Map();
  for (const g of r.governed) { const k = g.fact.cid + '|' + g.pid; const e = m.get(k) || { g, n: 0, ok: 0, inChange: 0 }; e.n++; if (g.conforms) e.ok++;
    if (!touched || touched(g.line, g.endLine || g.line)) e.inChange++; m.set(k, e); }
  return m; }
...
async function fileFindings({ root, rel, isGit, dirty, r, wholeFile = false, diffArgs }) {
  ...
  const waiveIn = (r.waiverHits || []).filter(h => !touched || touched(h.line, h.endLine)), waivePre = (r.waiverHits || []).filter(h => touched && !touched(h.line, h.endLine));
  const archIn = (r.archHits || []).filter(h => !touched || touched(h.line, h.line)), archPre = (r.archHits || []).filter(h => touched && !touched(h.line, h.line));
  const lines = [...steerIn.map(h => h.text), ...waiveIn.map(h => h.text), ...(r.placeHit ? [r.placeHit.text] : []), ...archIn.map(h => h.text), ...inChange.map(g => g.text + `\n  (preference gap ${g.delta} bits)`)];
  return { touched, grouped, inChange, preOnly, steerIn, steerPre, waiveIn, waivePre, archIn, archPre, lines }; }
...
function fileVerdictJson({ rel, r, dirty, f, govFacts, stamp }) {
  ...
  return { schema: 'grain-check/1', file: rel, partition: r.partition, label: r.partition ? scopeLabel(r.partition) : null, scopes: scopesN, dirty, hasError: !!r.hasError,
    governed: [...govFacts.values()].map(e => ({ convention: r.partition + '::' + e.g.fact.cid + '::' + e.g.pid, label: e.g.label, statement: verbalize(e.g.fact, e.g.fact.exemplars.map(x => x.name)), established: e.g.fact.sraw, share: e.g.fact.share, scopes: e.n, conforming: e.ok, defining: !!e.g.defining })),
    ...
    waivers: (r.waiverHits || []).map(h => ({ waiver: h.id, pid: h.pid, expected: h.exp, observed: h.obs, scope: h.scope, kind: h.kind, line: h.line, inChange: !touched || touched(h.line, h.endLine) })),
...
export async function cmdCheck({ model, root, isGit, args, opts, stamp, store }) {
  ...
  const content = fromFlag !== undefined ? fromFlag : (refs ? refContent(root, refs.contentRef, rel) : undefined);
  if (fromFlag === undefined) { if (refs) { if (content == null) throw new Error(`no such file: ${rel} (not present at ${refs.contentRef || 'that ref'})`); }
    else if (!existsSync(join(root, rel))) throw new Error(`no such file: ${rel}`); }
  if (!EXT2GRAMMAR[extname(rel)]) { const ph = placementHit(model, rel); const dirty0 = fileDirty(root, rel, isGit, refs?.diffArgs);
    if (opts.json) return [JSON.stringify({ schema: 'grain-check/1', file: rel, noGrammar: extname(rel) || null, dirty: dirty0, placement: ph ? { token: ph.token, dir: ph.dir, statement: ph.text.replace(/^\[grain\] /, '') } : null, asOf: stamp(dirty0).replace(/^as of /, '') })];
  ...
  if (!r.partition) { const arch = (r.archHits || []).map(h => h.text);
    if (opts.json) return [JSON.stringify({ schema: 'grain-check/1', file: rel, noPartition: true, reason: r.reason, placement: r.placeHit ? { token: r.placeHit.token, dir: r.placeHit.dir, statement: r.placeHit.text.replace(/^\[grain\] /, '') } : null, architecture: (r.archHits || []).map(h => ({ kind: h.kind, to: h.to, line: h.line, seed: h.id || null })), asOf: stamp(dirty).replace(/^as of /, '') })];
  ...
  if (r.hasError && scopesN === 0) { // a real parse failure, not a genuinely trivial file — the ONLY case this branch may fire for now (it used to be permanently dead: r.scopes.length is never 0 because extractScopes always pushes a file-kind pseudo-scope)
    if (opts.json) return [JSON.stringify({ schema: 'grain-check/1', file: rel, parseFailed: true, hasError: true, asOf: stamp(dirty).replace(/^as of /, '') })];
  ...
  const { touched, inChange, preOnly, steerIn, steerPre, waiveIn, waivePre, archIn, archPre } = await fileFindings({ root, rel, isGit, dirty, r, wholeFile: fromFlag !== undefined, diffArgs: refs?.diffArgs });
  ...
  if (store) recordCheckFeedback(store, rel, r.partition, inChange, content ?? readFileSync(join(root, rel), 'utf8'));
  ...
  const inl = inLineForFile(model, opts.as || rel); if (inl) lines.push(inl);
  ...
  if (waivePre.length) lines.push(`  (${waivePre.length} waived departure(s) on lines you did not touch — \`--all\` shows${opts.all ? ':' : ''})`);
  ...
  if (newIn.length > 8) lines.push(`  (+${newIn.length - 8} more new-to-the-index group(s) in your change — \`grain check ${rel} --all\` for the rest)`);
  if (newPre.length) { const newPreCount = newPre.reduce((a, h) => a + (h.count || 1), 0);
    lines.push(`  (${newPreCount} more scope(s) elsewhere in this file were never indexed either — not in your change, \`--all\` shows${opts.all ? ':' : ''})`); }
  ...
  if (preOnly.length) { if (opts.all) for (const g of preOnly) lines.push(g.text + `\n  (preference gap ${g.delta} bits)`);
    ...
    else lines.push(`pre-existing (not in your change, not yours to fix — \`--all\` to list): ${preOnly.slice(0, 4).map(g => `${g.label}: ${g.summary || verbalize({ ...g, kind: g.kind }, g.exNames || []).replace(/ here /, ' ')} ×${g.pre}`).join(' · ')}${preOnly.length > 4 ? ` · +${preOnly.length - 4} more` : ''}`); }
  ...
  const ok = [...govFacts.values()].filter(e => e.ok === e.n && e.inChange > 0 && !(e.g.fact.exp === 'false' && /^auto\.(has|call|deco|extends|imp|stshape|returns):/.test(e.g.pid)));
  ...
  const defNote = e => e.g.defining ? ` — defines this group; grain enforces it on members, not on a non-member` : '';
  if (ok.length) lines.push(`conforms to: ${ok.slice(0, 6).map(e => `${e.g.label}: ${verbalize(e.g.fact, e.g.fact.exemplars.map(x => x.name))} (${pct(e.g.fact.share)}% of ${e.g.fact.sraw})${supNote(e)}${defNote(e)}`).join(' · ')}${ok.length > 6 ? ` · +${ok.length - 6} more` : ''}`);
  const knownFiles = new Set(model.partitions.flatMap(p => p.files));
  const files = [rel]; const newFileScopes = knownFiles.has(rel) ? {} : { [rel]: r.scopes };
...
export async function cmdReview({ model, root, isGit, args, opts, stamp, store }) {
  if (args.length) throw new Error('usage: grain review [--json] — no file argument; for one file, use `grain check <file>`');
  ...
  let anyDirty = false; const perFile = []; const newFileScopes = {}; const changedScopes = {};
  ...
  for (const rel of files) {
    ...
    if (!EXT2GRAMMAR[extname(rel)]) { const ph = placementHit(model, rel); // no grammar: only a placement signal can still speak (mirrors `check`'s no-grammar case)
      if (ph) perFile.push({ rel, dirty, r: null, f: { steerIn: [], waiveIn: [], archIn: [], inChange: [], preOnly: [], lines: [ph.text] } });
      continue; }
    let r; try { r = await checkFile({ model, root, rel, content, exemplarOk: existsMemo(root) }); }
    catch (e) { perFile.push({ rel, dirty, r: null, f: { steerIn: [], waiveIn: [], archIn: [], inChange: [], preOnly: [], lines: [`[grain] ${rel}: parse failed — skipped (${e.message})`] } }); continue; }
    ...
    if (store) recordCheckFeedback(store, rel, r.partition, f.inChange, content ?? readFileSync(join(root, rel), 'utf8'));
  ...
  const missing = missingLines(model, files, { sources: ['cochange', 'recipe', 'kin', 'shape'], newFileScopes, changedScopes }); // one renderer for "what's still missing" — co-change (same data/threshold as `completeness <file>`), a missing companion/registration recipe for a genuinely new file, §J3.2's kin gaps, and §J4.2's change-shape gaps
  if (opts.json) { const ccPartners = cochangeData(model, files).slice(0, 5).map(h => `${h.file}${h.dead ? ' (deleted)' : ''} (co-changed in ${h.sup}/${h.commits} commits)`); // JSON contract unchanged (§023: same shape, now honest about a dead partner) — same values `completenessDirectional` used to produce
    return [JSON.stringify({ schema: 'grain-check/1', files,
    ...
    missing: { kin: files.flatMap(rel => { const fsc = (changedScopes[rel] || []).find(s => s.kind === 'file'); // the same raw data the `kin:` lines render, read straight from the engine — never scraped back out of the text
      return fsc ? valueKinGaps(model, rel, fsc.vals, new Set(files)).map(g => ({ file: rel, ...g })) : []; }) },
  ...
  if (!totalFindings && !missing.length) lines.push(`clean — nothing to report across ${files.length} file${files.length === 1 ? '' : 's'} reviewed`);
...
function readSeeds(store) { const out = []; const boundaries = []; const waivers = [];
  if (existsSync(store.seedsPath)) for (const line of readFileSync(store.seedsPath, 'utf8').split('\n')) { const t = line.trim(); if (!t || t.startsWith('#')) continue;
    try { const j = JSON.parse(t);
      ...
      else if (j && j.waiver && j.waiver.path && j.waiver.pid) waivers.push({ id: j.id, path: j.waiver.path, name: j.waiver.name || '', pid: j.waiver.pid, note: j.note || '', author: j.author || '', createdAt: j.createdAt || '' });
...
const hashSeeds = ({ seeds, boundaries, waivers = [] }) => (seeds.length || boundaries.length || waivers.length) ? createHash('sha256').update(JSON.stringify([seeds, boundaries, waivers])).digest('hex').slice(0, 16) : '';
...
async function scopeSurfaces({ model, store, rel, name, root, isGit, opts }) {
  ...
  return { scope: s, surfaces: positive.map(([pid, v]) => ({ pid, value: v, statement: verb2({ pid, exp: v, kind: s.kind, heritageKind: heritageKindOf(pid, model) }, [s.name]) })) }; }
...
const cmdDecide = ctx => cmdSeed({ ...ctx, args: [DECIDE_SUBS[ctx.args[0]] || ctx.args[0], ...ctx.args.slice(1)] });
async function cmdSeed({ model, root, isGit, store, args, opts, stamp }) {
  const sub = args[0];
  if (sub === 'list') { const { seeds, boundaries, waivers } = readSeeds(store); if (!seeds.length && !boundaries.length && !waivers.length) return ['no seeds — `grain seed add <path>#<name> --surfaces <pid,…> --note "…"` (pattern) or `grain seed add-boundary <from> --never-imports <to> --note "…"` (architecture) records a maintainer decision in .grain/seeds.jsonl', stamp()];
    return [...seeds.map(sd => `${sd.id}  ${sd.path}#${sd.name}  ${sd.pids.join(',')}  weight ${sd.weight}${sd.topic ? `  topic "${sd.topic}"` : ''}${sd.note ? `  — ${sd.note}` : ''}  (${[sd.author, sd.createdAt].filter(Boolean).join(' ')})`),
      ...boundaries.map(bd => `${bd.id}  boundary: ${bd.boundary.from}/ never imports ${bd.boundary.to}/${bd.note ? `  — ${bd.note}` : ''}  (${[bd.author, bd.createdAt].filter(Boolean).join(' ')})`),
      ...waivers.map(wv => `${wv.id}  waiver: ${wv.path}#${wv.name} on ${wv.pid}${wv.note ? `  — ${wv.note}` : ''}  (${[wv.author, wv.createdAt].filter(Boolean).join(' ')})`), stamp()]; }
  ...
  if (sub === 'add') { const target = args[1]; if (!target) throw new Error('usage: grain seed add <path>[#<scope name>] [--surfaces <pid,…>] [--instead-of <pid,…>] [--weight N] [--topic "…"] [--note "…"] [--author X]');
    ...
    return [`recorded seed ${id} in .grain/seeds.jsonl — ${want.map(pid => verb2({ pid, exp: r.scope.preds[pid], kind: r.scope.kind, heritageKind: heritageKindOf(pid, model) }, [r.scope.name])).join('; ')} (weight ${rec.weight}, capped at half the real population of each cell). Commit .grain/seeds.jsonl and .grain/decisions.jsonl; the next query re-mines with it.`, stamp()]; }
  ...
  if (sub === 'waive') { const target = args[1]; const pid = typeof opts.on === 'string' ? opts.on : null;
    if (!target || !pid) throw new Error('usage: grain decide waive <path>#<name> --on <pid> --note "…" [--author X]');
    const [p0, name] = target.split('#'); const rel = relPath(root, p0);
    if (!name) throw new Error(`grain decide waive names one scope: ${rel}#<scope name> (a waiver excuses a scope, never a whole file)`);
    const all = await loadScopes({ root, isGit, store, opts });
    const cands = all.filter(s => s.rel === rel && s.kind !== 'module' && s.name === name);
    if (!cands.length) return [`no scope named ${name} in ${rel}`, stamp()];
    if (cands.length > 1) return [`pick one scope of ${rel}: ${cands.map(x => `${x.kind} ${x.name} (line ${x.line})`).join(' · ')} — a waiver names exactly one, and \`${name}\` here names ${cands.length}`, stamp()];
    const createdAt = new Date().toISOString().slice(0, 10); const author = opts.author || process.env.USER || '';
    const id = createHash('sha256').update(['waiver', rel, name, pid, author, createdAt].join('|')).digest('hex').slice(0, 8);
    if (readSeeds(store).waivers.some(wv => wv.id === id)) return [`waiver ${id} already exists for ${rel}#${name} on ${pid} — \`grain decide rm ${id}\` first to change it`, stamp()];
    mkdirSync(store.base, { recursive: true }); ensureSeedAttrs(store);
    appendFileSync(store.seedsPath, JSON.stringify({ id, waiver: { path: rel, name, pid }, note: opts.note || '', author, createdAt }) + '\n');
    appendDecision(store, { action: 'add', id, at: createdAt, by: author, note: opts.note || '' });
    return [`recorded waiver ${id} in .grain/seeds.jsonl — ${rel}#${name} (line ${cands[0].line}) is excused from ${pid}. \`check\` will say the departure is deliberate instead of flagging it; the counts still report it as non-conforming. Commit .grain/seeds.jsonl and .grain/decisions.jsonl.`, stamp()]; }
  ...
  throw new Error('usage: grain decide steer|boundary|waive|list|rm … (grain seed add|add-boundary|list|rm is the same command)'); }
...
async function cmdExport({ model, meta, head, root, isGit, args, opts, stamp, store }) {
  if (args.length) throw new Error('usage: grain export [--out <file>] [--max-sites N] [--compact] [--no-anchors] — takes no arguments');
...
export async function cmdStatus({ model, meta, head, isGit, stamp, args, opts, store, treeDirty }) {
  ...
  return [...statusLines(model), `signal: ${sig.facts} conventions over ${sig.files} source files — ${sig.verdict}`, ...placementOutcomeLine(store), ...checkOutcomeLine(store), ...freshnessLines(meta, head, isGit), ...(treeDirty ? [DIRTY_TREE_NOTE] : []), stamp()]; }
...
function checkOutcomeLine(store) {
  ...
  return total ? [`check notes acted on: ${out.acted} of ${total} (${Math.round(out.acted / total * 100)}%)`] : []; }
export async function cmdReport({ model, meta, head, isGit, args, opts, stamp, store, treeDirty }) {
  ...
  return [...report(model, { top: +opts.top || 15, outcomes }), ...freshnessLines(meta, head, isGit), ...(treeDirty ? [DIRTY_TREE_NOTE] : []), stamp()]; }
...
async function cmdRules({ model, isGit, head, args, opts, stamp, store, treeDirty }) {
  ...
  const text = rulesMarkdown(model, { top: +opts.top || 15, sha: short(isGit ? head : null), date: new Date().toISOString().slice(0, 10), outcomes, dirty: treeDirty }).join('\n');
  if (opts.out) { const p = isAbsolute(opts.out) ? opts.out : resolve(process.cwd(), opts.out); atomicWrite(p, text + '\n');
...
export function sessionContext({ root, isGit, store, mode }) {
  ...
  const text = [
    ...
    ...(model && model.concepts && model.concepts.length ? [voice('map', `concepts: ${model.concepts.join(', ')}`)] : []),
    ...(model && model.changeArchetypes && model.changeArchetypes.length ? [(() => { const cs = model.changeArchetypes;
      const segs = cs.slice(0, 4).map(a => `"${a.label}" — ${a.n} change${a.n === 1 ? '' : 's'}`);
      return voice('practiced', `changes: ${segs.join(' · ')}${cs.length > 4 ? ` · +${cs.length - 4} more` : ''}`); })()] : []),
...
function seenGate(store, key, sigText) {
  try {
    ...
    const p = join(store.dir, 'hook-seen.json'); const seen = readJson(p) || {};
    ...
    return speak; }
  catch { return true; /* stateless is still correct, just louder */ } }
...
function checkPendingKey(rel, factKey) { return rel + '#' + factKey; }
function recordCheckFeedback(store, rel, partition, inChange, text) {
  try {
    const now = Date.now(); const ttl = +(process.env.GRAIN_HOOK_TTL_MS || 15 * 60 * 1000);
    const p = join(store.dir, 'check-pending.json'); const pending = readJson(p) || {};
    prunePending(pending, now, ttl);
    const h = createHash('sha256').update(text).digest('hex').slice(0, 16);
    const prefix = rel + '#';
    let acted = 0, ignored = 0; const ignoredFacts = [];
    for (const k of Object.keys(pending)) {
      ...
      if (!g) { acted++; delete pending[k]; continue; } // the deviation is gone — no group with this (rel, factKey) remains
      if (pending[k].h !== h) { ignored++; ignoredFacts.push(partition + '::' + g.pid); delete pending[k]; }
      // else: same deviation, unchanged content — hasn't had a chance to act yet, leave it pending untouched
    }
    for (const g of inChange) { const k = checkPendingKey(rel, g.factKey); if (!pending[k]) pending[k] = { t: now, obs: g.obs, h }; }
    writeFileSync(p, JSON.stringify(pending));
    if (acted || ignored) {
      const op = join(store.dir, 'check-outcomes.json'); const outcomes = readJson(op) || { acted: 0, ignored: 0, byFact: {} };
      outcomes.acted += acted; outcomes.ignored += ignored;
...
function capReviewLines(lines, sectionCap, missingCap) {
  ...
  const sections = [];
  for (const l of lines.slice(1, bodyEnd)) { if (/^== .+ — \d+ finding\(s\) ==$/.test(l)) sections.push([l]); else if (sections.length) sections[sections.length - 1].push(l); }
  const missing = missingIdx === -1 ? [] : lines.slice(missingIdx + 1, lines.length - 1);
  const out = [lines[0]];
  for (const s of sections.slice(0, sectionCap)) out.push(...s);
  if (sections.length > sectionCap) out.push(`  (+${sections.length - sectionCap} more file(s) — run \`grain review\`)`);
  if (missing.length) { out.push('missing from your change:');
    out.push(...missing.slice(0, missingCap));
    if (missing.length > missingCap) out.push(`  (+${missing.length - missingCap} more — run \`grain review\`)`); }
  out.push(stampLine);
  return out; }
...
export async function main(argv) {
  ...
  if (cmd === 'check-hook') { // PostToolUse hook: grain comes to the agent after every edit — speaks ONLY when it has
    ...
    try {
      ...
      if (opts.pre) { // PreToolUse on Write: the file does not exist yet — placement speaks from the PATH alone, BEFORE
        ...
        const lines = await cmdCheck({ model: model2, root: f.root, isGit: f.git, args: [rel], opts: {}, stamp: stamp2, store: st2 });
        ...
        if (cc.length) {
          ...
          if (seenGate(st2, 'cochange:' + rel, ccSig)) speak = [...speak, `[grain] ${voice('practiced', `edits like this also touch: ${cc.slice(0, 3).map(h => `${h.file}${h.dead ? ' (deleted)' : ''} (co-changed in ${h.sup}/${h.commits} commits)`).join(' · ')}`)}`];
        } }
    ...
    return 0; }
  if (cmd === 'edit-hook') { // PreToolUse hook on Edit|MultiEdit (§J6.4): co-change partners for the file about to
    ...
    try {
      let payload = null; try { if (!process.stdin.isTTY) { const raw = readFileSync(0, 'utf8'); if (raw.trim()) payload = JSON.parse(raw); } } catch { /* no payload → nothing to say */ }
      const fp = payload?.tool_input?.file_path || payload?.tool_input?.filePath || payload?.file_path;
      if (!fp) return 0;
      const f = findRoot({ repo: typeof payload?.cwd === 'string' ? payload.cwd : process.cwd() }); const st2 = storeFor(f.root);
      const meta2 = readJson(st2.metaPath); const model2 = meta2 && existsSync(st2.modelPath) ? readJson(st2.modelPath) : null;
      ...
      const text = `[grain] ${voice('practiced', `before you edit ${rel}, note: edits like this also touch: ${cc.slice(0, 3).map(h => `${h.file}${h.dead ? ' (deleted)' : ''} (co-changed in ${h.sup}/${h.commits} commits)`).join(' · ')}`)}`;
      console.log(JSON.stringify({ hookSpecificOutput: { hookEventName: 'PreToolUse', additionalContext: text } })); }
    catch (e) { if (process.env.GRAIN_DEBUG) console.error('[grain] edit-hook: ' + (e?.stack || e)); /* a hook never blocks an edit */ }
    return 0; }
  if (cmd === 'read-hook') { // PostToolUse hook on Read (§J6.2): the file just read is itself one of a fact's TOP-5
    ...
    try {
      ...
      for (const fct of part2.facts) { const dev = (fct.deviants || []).find(d => d.rel === rel); if (dev && dev.gap > bestGap) { bestGap = dev.gap; best = { fct, dev }; } }
      if (!best) return 0;
      const { fct, dev } = best;
      ...
      const text = `[grain] ${voice('practiced', `note: this file departs from its group on ${verbalize(fct, fct.exemplars.map(e => e.name))} (line ${dev.line}) — don't copy that part; a conforming sibling: ${ptr(near.rel, near.line, near.endLine)} \`${near.name}\`${skipLineNote(part2, fct, near)} — ${conformN}/${fct.sraw} established do it the other way`)}`;
    ...
    catch (e) { if (process.env.GRAIN_DEBUG) console.error('[grain] read-hook: ' + (e?.stack || e)); /* a hook never breaks a read */ }
    return 0; }
  if (cmd === 'how-hook') { // UserPromptSubmit hook: `how` speaks unbidden ONLY when the prompt itself resembles a
    ...
    try {
      ...
      const meta2 = readJson(st2.metaPath); const model2 = meta2 && existsSync(st2.modelPath) ? readJson(st2.modelPath) : null;
      if (!model2 || meta2.engine !== ENGINE_VERSION || meta2.extractor !== EXTR_V || (meta2.model || '') !== MODEL_V) return 0; // stale schema: silence, the next real query rebuilds
      const head = headSha(f.root); if (!head) return 0;
      ...
      const { matches, places, shape } = howCmd({ model: model2, H, query: prompt, top: 3, msgOf: null, shapes: true, exemplarOk: existsMemo(f.root) });
      ...
      const lines = [];
      if (certified) lines.push(`certified shape "${shape.label}" (${shape.n} changes): ${shape.cells.map(c => `${archCellLabel(model2, c.cell)} (${c.k} of ${shape.n})`).join(' · ')}`);
      ...
      if (relPlaces.length) { lines.push('places such a change touched:');
        for (const p of relPlaces) lines.push(`  ${p.rel} (${p.k}/${p.of}) — ${p.exists ? p.module : '(deleted)'}`); }
      ...
      const key = 'how:' + createHash('sha256').update(matches.map(m => m.sha).sort().join(',')).digest('hex').slice(0, 16);
    ...
    catch (e) { if (process.env.GRAIN_DEBUG) console.error('[grain] how-hook: ' + (e?.stack || e)); /* a hook never breaks a prompt */ }
    return 0; }
  if (cmd === 'commit-hook') { // PreToolUse hook on Bash (§J6.3): a `git commit` about to run — review the change
    ...
    try {
      let payload = null; try { if (!process.stdin.isTTY) { const raw = readFileSync(0, 'utf8'); if (raw.trim()) payload = JSON.parse(raw); } } catch { /* no payload → nothing to review */ }
      ...
      if (!m) return 0;
      const tailEnd = (() => { const rel = command.slice(m.index + m[0].length).search(/[;&|\n]/); return rel === -1 ? command.length : m.index + m[0].length + rel; })();
      ...
      const stamp2 = d => `as of ${short(meta2.headSha)}${d ? '+dirty' : ''}`;
      const lines = await cmdReview({ model: model2, root: f.root, isGit: f.git, args: [], opts: reviewOpts, stamp: stamp2, store: st2 });
      ...
      console.log(JSON.stringify({ hookSpecificOutput: { hookEventName: 'PreToolUse', additionalContext: text } })); }
    catch (e) { if (process.env.GRAIN_DEBUG) console.error('[grain] commit-hook: ' + (e?.stack || e)); /* a hook never blocks a commit */ }
    return 0; }
  if (cmd === 'version') { if (args.length) throw new Error('usage: grain version — takes no arguments');
    console.log(`grain ${ENGINE_VERSION} · extractor ${EXTR_V} · grammars ${GRAMMARS.join(', ')}`); return 0; }
  ...
  switch (cmd) {
    ...
    case 'how': lines = await cmdHow(ctx); break;
    case 'what': lines = await cmdWhat(ctx); break;
    case 'map': lines = await cmdMap(ctx); break;
    case 'check': lines = args.length === 0 ? await cmdReview(ctx) : await cmdCheck(ctx); break; // no file argument: `check` is an alias of `review` — the whole uncommitted change (J1.1)
    case 'review': lines = await cmdReview(ctx); break;
    case 'spectrum': case 'explain': lines = await cmdSpectrum(ctx); break;
    ...
    case 'decide': case 'seed': lines = await cmdDecide(ctx); break;
    case 'refresh': if (args.length) throw new Error('usage: grain refresh [--full] — takes no arguments');
      lines = [...statusLines(model), ...freshnessLines(meta, head, isGit), stamp()]; break;
    case 'completeness': lines = [...completenessDirectional(model, args.map(a => relPath(root, a))), ...(treeDirty ? [DIRTY_TREE_NOTE] : []), stamp()]; break;
    ...
    case 'selftest': {
      ...
      if (opts.where) { // J2.3's sibling: `where` ranked against the repository's own record of where an added file landed
        ...
        if (!H || !H.fps || !H.fps.length) {
          ...
          lines = opts.json ? [JSON.stringify({ note, where: null, base: null, unnamed: null, n: 0, silent: 0, asOf: stamp().replace(/^as of /, '') })] : [note, stamp()];
          break; }
        const res = whereEval({ model, H, last: +opts.last || 100 });
        if (opts.json) lines = [JSON.stringify({ ...res, asOf: stamp().replace(/^as of /, '') }, null, 1)];
        else { const f = x => x.toFixed(2); const armLine = a => `hit@3=${f(a.hit3)} MRR=${f(a.mrr)} place@3=${f(a.place3)}`;
          lines = [`where: ${armLine(res.where)} · path-match baseline: ${armLine(res.base)} · n=${res.n} · nothing-ranked=${res.silent}`,
            `query does not name the file (n=${res.unnamed.n}) — where: ${armLine(res.unnamed.where)} · baseline: ${armLine(res.unnamed.base)}`, stamp()]; }
        break; }
      if (opts.how) { // BRAMKA J2.3: leave-one-out P/R of `how` vs a grep baseline, over the repo's own history — never the mutate-test path
        let H = null; if (isGit) { try { H = (await loadHistory({ gitdir: root, store, log })).H; } catch (e) { log('history unavailable for selftest --how: ' + e.message); } }
        if (!H || !H.fps || !H.fps.length) {
          ...
          lines = opts.json ? [JSON.stringify({ note, how: null, grep: null, n: 0, noMatch: 0, asOf: stamp().replace(/^as of /, '') })] : [note, stamp()];
          break; }
        const res = howEval({ model, H, root, last: +opts.last || 100 });
        if (opts.json) lines = [JSON.stringify({ ...res, asOf: stamp().replace(/^as of /, '') }, null, 1)];
        else { const f = x => x.toFixed(2);
          lines = [`how: P=${f(res.how.meanP)} R=${f(res.how.meanR)} F1=${f(res.how.meanF1)} (median P=${f(res.how.medP)} R=${f(res.how.medR)} F1=${f(res.how.medF1)}) · grep: P=${f(res.grep.meanP)} R=${f(res.grep.meanR)} F1=${f(res.grep.meanF1)} (median P=${f(res.grep.medP)} R=${f(res.grep.medR)} F1=${f(res.grep.medF1)}) · n=${res.n} · no-match=${res.noMatch}`, stamp()]; }
        break; }
      const res = await mutateTest({ model, root });
      if (opts.json) lines = [JSON.stringify({ ...res, asOf: stamp().replace(/^as of /, '') }, null, 1)];
      else { const plantable = res.detected + res.missed;
        lines = [`selftest: ${res.detected}/${plantable} planted deviations caught · ${res.falseFire} false fires · ${res.unsupported} unsupported`, stamp()]; }
      break; }
