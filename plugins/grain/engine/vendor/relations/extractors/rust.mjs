...
// Source: Yggdrasil 1f82edbb6 source/cli/src/relations/extractors/rust.ts (MIT, same author). Regenerate: node scripts/build-relations.mjs
...
function rebaseForInlineMods(specifier, names) {
    const segs = specifier.split('::');
    if (segs[0] === 'self') return [
        'self',
        ...names,
        ...segs.slice(1)
    ].join('::');
    ...
    if (supers <= names.length) {
        return [
            'self',
            ...names.slice(0, names.length - supers),
            ...rest
        ].join('::');
    }
    return [
        ...new Array(supers - names.length).fill('super'),
        ...rest
    ].join('::');
}
            ...
            case 'scoped_use_list':
                {
                    ...
                    if (groupPrefix === undefined) return;
