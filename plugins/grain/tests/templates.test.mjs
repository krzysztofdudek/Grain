...
// `report`'s template lines look like established practice (a skeleton "held since", a per-instance slot
...
// rules, byte-identically (the rules-markdown parity check's drift is the failure mode this guards against). The companion assertion — a
...
test('both report and rules mark the template line descriptive, byte-identically', () => {
