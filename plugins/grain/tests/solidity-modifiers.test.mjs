...
test('043: the modifier certifies as a convention of its own', () => {
  ...
  // Solidity has no `@` sigil at all, so the pid and every rendering of it stay bare — `onlyGuard`, not `@onlyGuard`.
...
// ---- no other surface reconstructs an `@` this language never had ----
...
// (buildCards' `label`/`mpid`, the sigil call) is the one other real surface, so it is exercised here instead.
...
`));
  ...
  // no `@` — Solidity modifiers are written bare in real source (`function f() onlyGuard { ... }`)
