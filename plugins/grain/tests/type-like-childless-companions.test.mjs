// the SAME childless-companion bug the Scala-object fix closed for Scala's bodiless `object`, recurring in five more node
// types that the Scala-object fix's own tests/type-like-coverage.test.mjs surfaced: Java/Groovy `module_declaration`, Ruby
...
//
// The fix (core.mjs, TYPE_LIKE_RE) adds the bare words `module` and `library` — same technique as the Scala-object fix's `object`
...
//
// This same type-like coverage fix also removes the pre-existing false positive where Ruby's `singleton_method` (`def
...
test('(Java): a module-info declaration with no nested class/method classifies as type, not method', async () => {
...
test('(Groovy): the same module-info construct classifies as type', async () => {
...
test('(Ruby): a `module` holding only constants (no nested class/def) classifies as type, not method', async () => {
...
test('(TypeScript): a `namespace Foo { ... }` (internal_module) holding only a const classifies as type', async () => {
...
test('(TypeScript): a `module Foo { ... }` holding only a const classifies as type', async () => {
...
test('(Solidity): a `library` holding only a constant classifies as type, not method', async () => {
...
test('control: a Java class with only fields (no module involved) was already type — unaffected by this fix', async () => {
