// `where` returned a confident, specific, WRONG top hit whenever the query was a COMPOUND identifier whose
...
//   · the never-parsed note's never-parsed disclosure is gated on `!hits.length` (grain.mjs `cmdWhere`) — hits are non-empty here;
//   · the zero-foothold banner's zero-foothold banner needs every hit's `lex0 === 0` — the fragments give real content overlap;
...
test('a compound identifier the model never declares is ranked from its FRAGMENTS — and that is exactly when every existing honest-negative check stands down', () => {
  ...
  assert.ok(res.hits.length > 0, 'the never-parsed note cannot fire: hits are NOT empty');
  assert.ok(res.hits[0].lex0 > 0, 'the zero-foothold banner cannot fire: the fragments give real content overlap, so lex0 is off zero');
...
test('red→green: given the never-parsed hit, the ranked answer now discloses that the real text lives in a file grain cannot read', () => {
  const model = fragmentModel();
  const res = whereCmd({
    model,
    query: 'indent_style',
...
test('control: a name the model DOES declare never asks for the scan, even when its text also sits in an unparsed file', () => {
  const part = {
    name: '_root',
    medoids: [],
    assignments: {},
...
test('control: a plain English word is not identifier-shaped, so `where users` never pays for the scan and never disclaims', () => {
...
// the never-parsed note's scan discipline: "every other query, including every one with an exact-name hit, never opens a file at
// all". A multi-word intent must not start a file scan — the never-parsed note matches the query VERBATIM, so a sentence can never
...
test('keeps the never-parsed note\'s scan discipline: a multi-word intent never asks for the never-parsed scan', () => {
  const model = fragmentModel();
...
// The the unknown-identifier path banner runs AFTER the existing ladder, never before it. The ladder's last arm can SUPPRESS an
...
// had been discarding came back. A suppressed answer must stay suppressed and fall through to the never-parsed note's own message,
// which names the same never-parsed file anyway.
test('never revives a ranking the ladder suppressed — the stronger honest negative still wins', () => {
  ...
  if (!before.hits.length)
    assert.doesNotMatch(
      after.lines.join('\n'),
      /is not a name grain parsed anywhere/,
      'a suppressed answer gets the never-parsed note\'s message, not a banner over a revived ranking'
    );
});

test('does not disturb the zero-foothold banner: an identifier pin with zero content foothold still gets the zero-foothold banner', () => {
  const part = {
    name: '_root',
    medoids: [],
    assignments: {},
  ...
  assert.equal(res.unknownIdent, false, 'the card declares `isNew`, so the unknown-identifier path stands down and the zero-foothold banner keeps the case');
