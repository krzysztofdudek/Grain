// `what`'s `siblings:` line is deleted. Measured (see the maintainer notes' log):
...
//     `valueSiblings` field is that data verbatim — the precedent for `model.twins` exactly.
...
test('(a) the sibling-values deletion: `what` renders NO siblings: line, while its other sources still render', () => {
  ...
  assert.equal(
    ...
    undefined,
    `the sibling-values deletion: the siblings: line is deleted — got:\n${r.out}`
  );
...
test('(b) the sibling-values deletion: `what --json` carries no `siblings` field, and its other fields are intact', () => {
  const r = grainIn(repo, ['what', 'status', '--json']);
  assert.equal(r.code, 0, r.err);
  const j = JSON.parse(r.out);
  assert.ok(!('siblings' in j), `the sibling-values deletion: the JSON siblings field is deleted — got keys ${Object.keys(j).join(',')}`);
...
test('(c) the sibling-values deletion: the deletion is a RENDERING decision — the container is still fully certified in the model', () => {
...
test('(e) both arms — the published export schema still documents valueSiblings', () => {
  ...
  assert.match(
    ...
    /value concordance/i,
    'the export schema note is a published interface and is not touched by the sibling-values deletion'
  );
});
