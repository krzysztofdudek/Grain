...
//      a member-expression heritage shape, the same class as the constructor-argument bug — that is now a
//      NO-FALSE-POSITIVE case: it found the qualified-heritage bug (which this instrument itself flagged) and
...
before(() => {
  ...
  // failure class as the constructor-argument bug (wrong identifier out of a compound heritage clause); the qualified-heritage fix closed this shape too (a
  // qualified/member heritage node now resolves to its LAST name-shaped child, structurally, for every grammar
  // that has the shape). This fixture is kept as the qualified-heritage fix's own regression guard: it must go on producing NO
...
test('end-to-end (regression guard for the qualified-heritage fix): `extends lib.Base1` is checked and produces NO fabrication naming `lib`', () => {
  ...
  // flagged `lib` as fabricated (undeclared, not import-shaped, not type-shaped). The qualified-heritage fix changed the extraction to
  ...
  assert.deepEqual(heritageJs, [], `expected no heritageTargetReal fabrication for src/heritage.js after the qualified-heritage fix: ${JSON.stringify(heritageJs)}`);
  assert.ok(!out.samples.some(s => s.type === 'heritageTargetReal' && s.claim.includes('`lib`')),
    'the namespace `lib` must never be the recorded heritage target again — that was exactly the bug');
...
// the exact bug a worker's escalation reported: `where --json` used to carry no disclosure text at
// all, so this instrument kept counting a confident-but-DISCLOSED hit as silent fabrication even after `where`'s
// own text answer had already told the reader the real text sits in a file grain cannot read. Now
// that --json carries the identical { kind: 'ungrammared', text } entry the text renderer emits, the SAME
// confident-wrong hit as the red test above must no longer be flagged — this is red BEFORE the JSON disclosures' `disclosed`
// check (proved by the sibling test above, whose mock carries no `disclosures` field) and green after it.
test('noDeclarationsAnywhere: a confident hit that ALSO carries a matching "ungrammared" disclosure is NOT counted as fabrication', () => {
  const cacheDir = mkdtempSync(join(tmpdir(), 'audit-claims-cache-'));
  mkdirSync(join(cacheDir, '.grain', 'cache'), { recursive: true });
  writeFileSync(join(cacheDir, '.grain', 'cache', 'model.json'), JSON.stringify({ filesAll: ['src/a.ts'], pathsAll: ['src/a.ts', 'schema.xml', 'schema2.xml'] }));
  ...
  try {
    ...
    // response also carries the disclosures[] entry the JSON disclosures added to `where --json`
    const res = checkNoDeclarationsAnywhere({}, cacheDir, corpus, { whereQueries: 5 }, () => ({
      hits: [{ type: 'file', label: 'src/a.ts', score: 0.8, members: [{ rel: 'src/a.ts' }] }],
...
// hit — the JSON disclosures only suppresses the exact class of caveat that actually applies to this check's own sampling shape
