...
// it: ruby-external-stdlib-constant-silence
...
//
// Rule (reference/relations/ruby/ruby-external-stdlib-constant-silence.md): a constant that
// names a gem or core/stdlib type (`Math::PI`, `StandardError`) is emitted as an ordinary
// symbol use, but the SymbolTable has ZERO in-graph definitions → absent → silence.
...
test('ruby-external-stdlib-constant-silence', async () => {
  const files = [{ path: 'src/app/calculator.rb', language: 'ruby', code: 'x = Math::PI\ny = StandardError\n' }];
  const expectEdges = [];
