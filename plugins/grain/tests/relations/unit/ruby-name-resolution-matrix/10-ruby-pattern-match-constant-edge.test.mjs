...
// it: ruby-pattern-match-constant-edge
...
//
// Rule (reference/relations/ruby/ruby-pattern-match-constant-edge.md): a pattern may name a
// constant for a type/deconstruct match (`in Errors::NotFound`) — an ordinary
// constant/scope_resolution node reached by generic descent, same resolveUnique/C1 handling
// as any value-use constant.
...
test('ruby-pattern-match-constant-edge', async () => {
  const files = [
    { path: 'src/errors/not_found.rb', language: 'ruby', code: 'module Errors\n  class NotFound\n  end\nend\n' },
    { path: 'src/handler/dispatch.rb', language: 'ruby', code: 'case x\nin Errors::NotFound\n  handle\nend\n' },
  ];
  const expectEdges = [{ fromFile: 'src/handler/dispatch.rb', line: 2, node: 'errors' }];
