...
// it: ruby-dynamic-const-get-silence
...
//
// Rule (reference/relations/ruby/ruby-dynamic-const-get-silence.md): `const_get`/`autoload`
// route a constant through a string/symbol node, never a `constant` node, so the dynamic
// target is never emitted; the `Object` receiver is stdlib → unmapped → silenced.
...
test('ruby-dynamic-const-get-silence', async () => {
  const files = [
    { path: 'src/registry/widget.rb', language: 'ruby', code: 'class Widget\nend\n' },
    {
      path: 'src/app/dynamic.rb',
      language: 'ruby',
      code: "obj = Object.const_get('Widget')\nautoload :Widget, 'widget'\n",
    },
  ];
  const expectEdges = [];
