...
// it: ruby-bare-constant-value-edge
...
//
// Rule (reference/relations/ruby/ruby-bare-constant-value-edge.md): a bare constant used as a
// value (`x = Helper`) at TOP LEVEL has no enclosing namespace to shadow it, so it is taken
// as the FQN key and emitted.
...
test('ruby-bare-constant-value-edge', async () => {
  const files = [
    { path: 'src/helpers/helper.rb', language: 'ruby', code: 'class Helper\nend\n' },
    { path: 'src/app/runner.rb', language: 'ruby', code: 'x = Helper\n' },
  ];
  const expectEdges = [{ fromFile: 'src/app/runner.rb', line: 1, node: 'helpers' }];
