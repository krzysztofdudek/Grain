...
//
// Rule (reference/relations/ruby/ruby-superclass-edge.md): `class C < Base` names `Base` as
// the superclass, resolved through the shared SymbolTable by its lexically-built FQN; a
// uniquely-defined `BaseService` in node `base` makes `OrderService < BaseService` an edge.
...
test('ruby-superclass-edge', async () => {
  const files = [
    { path: 'src/base/base_service.rb', language: 'ruby', code: 'class BaseService\nend\n' },
    { path: 'src/orders/order_service.rb', language: 'ruby', code: 'class OrderService < BaseService\nend\n' },
  ];
  const expectEdges = [{ fromFile: 'src/orders/order_service.rb', line: 1, node: 'base' }];
