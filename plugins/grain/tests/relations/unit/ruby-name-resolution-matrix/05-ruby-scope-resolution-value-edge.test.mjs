...
// it: ruby-scope-resolution-value-edge
...
//
// Rule (reference/relations/ruby/ruby-scope-resolution-value-edge.md): a scope_resolution
// value (`x = Payments::Gateway`) is a complete reference; only the full path is emitted, not
// the inner qualifier segments.
...
test('ruby-scope-resolution-value-edge', async () => {
  const files = [
    { path: 'src/payments/gateway.rb', language: 'ruby', code: 'module Payments\n  class Gateway\n  end\nend\n' },
    { path: 'src/app/charge.rb', language: 'ruby', code: 'x = Payments::Gateway\n' },
  ];
  const expectEdges = [{ fromFile: 'src/app/charge.rb', line: 1, node: 'payments' }];
