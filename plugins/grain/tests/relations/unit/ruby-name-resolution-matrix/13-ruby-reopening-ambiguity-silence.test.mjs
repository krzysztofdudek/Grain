...
// it: ruby-reopening-ambiguity-silence
...
//
// Rule (reference/relations/ruby/ruby-reopening-ambiguity-silence.md): `class Foo … end` in
// two files both REOPEN/define `Foo`; the SymbolTable then has 2 files for `Foo` →
// resolveUnique undefined → any use of `Foo` is SILENCED (the central recall-killer).
...
test('ruby-reopening-ambiguity-silence', async () => {
  const files = [
    { path: 'src/x/widget.rb', language: 'ruby', code: 'class Widget\nend\n' },
    { path: 'src/y/widget.rb', language: 'ruby', code: 'class Widget\nend\n' },
    { path: 'src/z/use.rb', language: 'ruby', code: 'x = Widget\n' },
  ];
  const expectEdges = [];
