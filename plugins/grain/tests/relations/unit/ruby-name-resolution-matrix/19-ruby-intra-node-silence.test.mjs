...
//
// Rule (reference/relations/ruby/ruby-intra-node-silence.md): a reference whose resolved
// target maps to the SAME node as the referrer is not a cross-node dependency — a
// same-directory require_relative and a same-node superclass both collapse to no edge.
...
test('ruby-intra-node-silence', async () => {
  const files = [
    { path: 'src/orders/base.rb', language: 'ruby', code: 'class Base\nend\n' },
    {
      path: 'src/orders/order.rb',
      language: 'ruby',
      code: "require_relative './base'\nclass Order < Base\nend\n",
    },
  ];
  const expectEdges = [];
