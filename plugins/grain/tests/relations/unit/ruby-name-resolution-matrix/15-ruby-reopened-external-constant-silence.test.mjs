...
// it: ruby-reopened-external-constant-silence
...
//
// Rule (reference/relations/ruby/ruby-reopened-external-constant-silence.md): a compact-form
// declaration `module Rack::Handler` reopens a namespace `Rack` that has NO bare in-repo
// declaration — `Rack` is external, so the resolver's root-anchoring guard treats
// `Rack::Handler` as unresolved and a reference to it does not bind the in-repo reopening.
...
test('ruby-reopened-external-constant-silence', async () => {
  const files = [
    { path: 'src/stub/server_stub.rb', language: 'ruby', code: 'module Rack::Handler\nend\n' },
    { path: 'src/app/runner.rb', language: 'ruby', code: 'x = Rack::Handler\n' },
  ];
  const expectEdges = [];
