...
// it: ruby-mixin-include-extend-prepend-edge
...
//
// Rule (reference/relations/ruby/ruby-mixin-include-extend-prepend-edge.md): `include` /
// `extend` / `prepend Mod` each name one or more modules to mix in; the extractor emits one
// symbol per constant argument, gated on the outer depth of the enclosing class/module.
...
test('ruby-mixin-include-extend-prepend-edge', async () => {
  const files = [
    { path: 'src/loggable/loggable.rb', language: 'ruby', code: 'module Loggable\nend\n' },
    { path: 'src/forward/forwardable.rb', language: 'ruby', code: 'module Forwardable\nend\n' },
    { path: 'src/tracing/hook.rb', language: 'ruby', code: 'module Tracing\n  module Hook\n  end\nend\n' },
    {
  ...
  const expectEdges = [
    { fromFile: 'src/widgets/widget.rb', line: 2, node: 'loggable' },
    { fromFile: 'src/widgets/widget.rb', line: 3, node: 'forward' },
    { fromFile: 'src/widgets/widget.rb', line: 4, node: 'tracing' },
  ];
