...
//
// Rule (reference/relations/ruby/ruby-assignment-rhs-edge.md): a top-level `MyAlias =
// OriginalClass` DEFINES `MyAlias` (LHS, resolves to own node → no self-edge) while the RHS
// `OriginalClass` is a USE reached by generic descent — the edge.
...
test('ruby-assignment-rhs-edge', async () => {
  const files = [
    { path: 'src/legacy/original_class.rb', language: 'ruby', code: 'class OriginalClass\nend\n' },
    { path: 'src/aliases/alias_def.rb', language: 'ruby', code: 'MyAlias = OriginalClass\n' },
  ];
  const expectEdges = [{ fromFile: 'src/aliases/alias_def.rb', line: 1, node: 'legacy' }];
