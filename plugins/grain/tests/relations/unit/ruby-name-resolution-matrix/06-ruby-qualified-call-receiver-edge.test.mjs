...
// it: ruby-qualified-call-receiver-edge
...
//
// Rule (reference/relations/ruby/ruby-qualified-call-receiver-edge.md): a `call` whose
// receiver is a constant/scope_resolution (`Payments::Gateway.charge`) references that
// constant; the method name is not a constant and emits nothing.
...
test('ruby-qualified-call-receiver-edge', async () => {
  const files = [
    { path: 'src/payments/gateway.rb', language: 'ruby', code: 'module Payments\n  class Gateway\n  end\nend\n' },
    { path: 'src/orders/handler.rb', language: 'ruby', code: 'Payments::Gateway.charge(amount)\n' },
  ];
  const expectEdges = [{ fromFile: 'src/orders/handler.rb', line: 1, node: 'payments' }];
