...
// Rule: files in the same directory share one package; a reference to a name declared
// in a sibling file of the same package needs NO import. With no `import_spec`, the
// import-only extractor emits nothing — and the two files map to the SAME node anyway,
// so even a detected reference would be intra-node, never a cross-node edge.
test('go-intra-package-silence', async () => {
  const files = [
    { path: 'm/go.mod', code: 'module example.com/m\n' },
    {
      path: 'm/billing/charge.go',
      code: 'package billing\nfunc Charge() { receipt() }\n',
      language: 'go',
    },
    { path: 'm/billing/receipt.go', code: 'package billing\nfunc receipt() {}\n', language: 'go' },
  ];
