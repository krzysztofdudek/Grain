...
// Rule: an in-module import resolves only when its package DIRECTORY is present in the
// graph. An import under the repo's own module that points at a directory carrying no
// mapped `.go` source has no representative file to point at → silence.
test('go-unmapped-package-silence', async () => {
  const files = [
