...
// Rule: a `go.work` workspace with `use (./a ./b)` lets a file in module `a` import
// module `b`'s packages by `b`'s OWN module path, but `go.work` adds no new import-path
// syntax. From the importing file the nearest go.mod is module `a`, and
// `example.com/b/lib` does not start with the `a` prefix → silence (go.work unmodeled).
test('go-go-work-workspace-silence', async () => {
  const files = [
    { path: 'ws/go.work', code: 'go 1.21\n\nuse (\n  ./a\n  ./b\n)\n' },
    { path: 'ws/a/go.mod', code: 'module example.com/a\n' },
    { path: 'ws/b/go.mod', code: 'module example.com/b\n' },
    { path: 'ws/b/lib/lib.go', code: 'package lib\nfunc Use() {}\n', language: 'go' },
    {
      path: 'ws/a/app/main.go',
