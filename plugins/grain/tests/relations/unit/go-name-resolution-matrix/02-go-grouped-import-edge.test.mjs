...
// Rule: a parenthesized `import ( … )` block holds N independent `import_spec`
// children; each is its own dependency, resolved by its own full path. Two in-module
// imports in one block produce one edge each, to the directory each path names.
test('go-grouped-import-edge', async () => {
  const files = [
    { path: 'm/go.mod', code: 'module example.com/m\n' },
    { path: 'm/billing/charge.go', code: 'package billing\nfunc Charge() {}\n', language: 'go' },
    { path: 'm/audit/log.go', code: 'package audit\nfunc Log() {}\n', language: 'go' },
    {
      path: 'm/app/main.go',
      code:
  ...
  expect(edges.sort()).toEqual([
    'm/app/main.go:3->node:billing',
    'm/app/main.go:4->node:audit',
  ].sort());
});
