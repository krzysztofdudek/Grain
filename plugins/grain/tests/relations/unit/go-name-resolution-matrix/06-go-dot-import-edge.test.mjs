...
// Rule: a dot import `import . "path"` merges the package's exported identifiers into
// the importing file's block (used unqualified). The `.` changes only how the names are
// spelled locally; it is a genuine, mandatory dependency on that package directory.
test('go-dot-import-edge', async () => {
  const files = [
    { path: 'm/go.mod', code: 'module example.com/m\n' },
    { path: 'm/dsl/builder.go', code: 'package dsl\nfunc Build() {}\n', language: 'go' },
    {
      path: 'm/app/main.go',
  ...
  expect(edges.sort()).toEqual(['m/app/main.go:2->node:dsl']);
});
