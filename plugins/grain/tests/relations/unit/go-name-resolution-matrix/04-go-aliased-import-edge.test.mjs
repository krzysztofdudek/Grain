...
// Rule: an aliased import `import alias "path"` renames only the LOCAL binding used in
// qualified identifiers; the extractor reads only the `path` field, never the alias, so
// the edge is to the import PATH — the alias token is never a target.
test('go-aliased-import-edge', async () => {
  const files = [
