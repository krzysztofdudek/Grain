// Reference-catalogue case `php-aliased-use-fqn-edge` (Yggdrasil reference/relations/php/php-aliased-use-fqn-edge.md), driven end-to-end
...
//
// Rule: `use App\Foo as Bar;` binds the LOCAL short name `Bar`, but the imported FQN is still
// `App\Foo`. The alias is a per-file rename, never the target — the extractor records
// `App\Payment\Gateway` and drops `G`. Emitting the alias (or `App\Payment\G`) would name
// nothing in the depended-on file and is a false positive the FQN key rejects by construction.
...
test("php-aliased-use-fqn-edge", async () => {
  const files = [
