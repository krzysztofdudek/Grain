// Reference-catalogue case `php-namespace-alias-use-edge` (Yggdrasil reference/relations/php/php-namespace-alias-use-edge.md), driven end-to-end
...
//
// Rule: `use App\Domain\Models;` binds the local name `Models`, which a downstream qualified
// usage `new Models\User()` could later prefix (→ `App\Domain\Models\User`, a separate
// usage-site form). The IMPORT line itself names exactly the FQN `App\Domain\Models` — that is
// the only emitted edge, and it points at the file declaring that FQN, never at the local name
// `Models`. Here `App\Domain\Models` happens to be a class declared in its own file; resolving
// the operand verbatim is the safe, FQN-keyed direction.
...
test("php-namespace-alias-use-edge", async () => {
  const files = [
