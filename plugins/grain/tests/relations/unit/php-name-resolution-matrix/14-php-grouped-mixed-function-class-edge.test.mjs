// Reference-catalogue case `php-grouped-mixed-function-class-edge` (Yggdrasil reference/relations/php/php-grouped-mixed-function-class-edge.md), driven end-to-end
...
//
// Rule: A grouped use may carry the `function` keyword on an INDIVIDUAL clause: `use
// App\Pkg\{function format, Gateway};`. Only that clause is a function import (no class edge);
// sibling CLASS clauses still import classes. Dropping the whole group on seeing one `function`
// keyword would lose the sibling class edge (a recall miss); emitting `format` as a class would
// be a false positive.
...
test("php-grouped-mixed-function-class-edge", async () => {
  const files = [
    { path: "src/Pkg/Gateway.php", language: "php", code: "<?php\nnamespace App\\Pkg;\nclass Gateway {}\n" },
  ...
  const expectEdges = [
    { fromFile: "src/Order/Handler.php", line: 3, node: "Pkg" },
  ];
