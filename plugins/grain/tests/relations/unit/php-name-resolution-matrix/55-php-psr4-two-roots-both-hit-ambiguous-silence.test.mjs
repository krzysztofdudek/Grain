// Reference-catalogue case `php-psr4-two-roots-both-hit-ambiguous-silence` (Yggdrasil reference/relations/php/php-psr4-two-roots-both-hit-ambiguous-silence.md), driven end-to-end
...
//
// Rule: When the class file exists under 2+ roots of one prefix, the FQN genuinely maps to two
...
test("php-psr4-two-roots-both-hit-ambiguous-silence", async () => {
  const files = [
    { path: "src/Pay/G.php", language: "php", code: "<?php\nnamespace App\\Pay;\nclass G {}\n" },
    { path: "lib/Pay/G.php", language: "php", code: "<?php\nnamespace App\\Pay;\nclass G {}\n" },
    { path: "src/Order/Handler.php", language: "php", code: "<?php\nnamespace App\\Order;\nuse App\\Pay\\G;\nclass Handler {}\n" },
  ];
  const configFiles = [
    { path: "composer.json", code: "{ \"autoload\": { \"psr-4\": { \"App\\\\\": [\"src/\", \"lib/\"] } } }\n" },
  ];
  const expectEdges = [];
