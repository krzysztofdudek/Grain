// Reference-catalogue case `php-inline-backslash-function-call-silence` (Yggdrasil reference/relations/php/php-inline-backslash-function-call-silence.md), driven end-to-end
...
//
// Rule: A leading-backslash name in FUNCTION-call position `\App\Util\format()` does NOT
// trigger class autoloading — PHP keeps functions in a separate namespace resolved at call
// time, never mapped to a PSR-4 class file. The extractor's class-autoload-position allowlist
// excludes function-call parents, so it emits nothing. Emitting it could bind to an unrelated
// class file that merely shares the path.
...
test("php-inline-backslash-function-call-silence", async () => {
  const files = [
    { path: "src/Util/format.php", language: "php", code: "<?php\nnamespace App\\Util;\nclass format {}\n" },
