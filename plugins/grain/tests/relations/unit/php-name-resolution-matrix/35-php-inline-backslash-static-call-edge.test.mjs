// Reference-catalogue case `php-inline-backslash-static-call-edge` (Yggdrasil reference/relations/php/php-inline-backslash-static-call-edge.md), driven end-to-end
...
//
// Rule: A static call on a leading-backslash class name `\App\Audit\AuditLog::record("x")` is
// an absolute class reference (Rule 1) — the qualified_name is the scope of a
// scoped_call_expression, a class-autoload position. It is shadow-free, so the extractor emits
// `App\Audit\AuditLog`.
...
test("php-inline-backslash-static-call-edge", async () => {
  const files = [
    { path: "src/Audit/AuditLog.php", language: "php", code: "<?php\nnamespace App\\Audit;\nclass AuditLog {}\n" },
  ...
  const expectEdges = [
    { fromFile: "src/Order/Handler.php", line: 3, node: "Audit" },
  ];
