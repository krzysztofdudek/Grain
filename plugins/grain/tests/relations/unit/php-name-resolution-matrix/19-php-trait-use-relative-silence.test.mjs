// Reference-catalogue case `php-trait-use-relative-silence` (Yggdrasil reference/relations/php/php-trait-use-relative-silence.md), driven end-to-end
...
//
// Rule: An in-class trait `use Timestamps;` (or `use Ns\Other;`) is namespace-RELATIVE, unlike
// the absolute top-level namespace `use`: in `namespace App` it resolves to `App\Timestamps` by
// current-namespace prepend, never global `\Timestamps`. The extractor never reads an in-class
// trait `use` (import-only), so it is silent — applying the absolute top-level rule here would
// be the T6 mis-bind.
...
test("php-trait-use-relative-silence", async () => {
  const files = [
    { path: "src/Mixin/Timestamps.php", language: "php", code: "<?php\nnamespace App\\Mixin;\ntrait Timestamps {}\n" },
