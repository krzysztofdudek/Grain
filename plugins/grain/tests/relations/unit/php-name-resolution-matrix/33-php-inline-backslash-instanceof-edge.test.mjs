// Reference-catalogue case `php-inline-backslash-instanceof-edge` (Yggdrasil reference/relations/php/php-inline-backslash-instanceof-edge.md), driven end-to-end
...
//
// Rule: `$x instanceof \App\M\Timer` with a leading backslash is an absolute class reference
// (Rule 1). The right operand of `instanceof` in a class-autoload position is shadow-free, so
// the extractor emits `App\M\Timer` and the resolver maps it by PSR-4.
...
test("php-inline-backslash-instanceof-edge", async () => {
  const files = [
    { path: "src/M/Timer.php", language: "php", code: "<?php\nnamespace App\\M;\nclass Timer {}\n" },
  ...
  const expectEdges = [
    { fromFile: "src/Order/Handler.php", line: 3, node: "M" },
  ];
