// Reference-catalogue case `php-grouped-use-one-edge-each` (Yggdrasil reference/relations/php/php-grouped-use-one-edge-each.md), driven end-to-end
...
//
// Rule: A grouped `use App\Payment\{Charge\Card, Refund\Cash};` is sugar for the individual
// imports `use App\Payment\Charge\Card;` and `use App\Payment\Refund\Cash;`. The leading base
// namespace is prepended to each clause, so each clause is its own per-type edge to its own
// resolved file. Forgetting the base would emit a bare clause that fails PSR-4 resolution;
// dropping a clause would lose its edge.
...
test("php-grouped-use-one-edge-each", async () => {
  const files = [
    { path: "src/Payment/Charge/Card.php", language: "php", code: "<?php\nnamespace App\\Payment\\Charge;\nclass Card {}\n" },
    { path: "src/Payment/Refund/Cash.php", language: "php", code: "<?php\nnamespace App\\Payment\\Refund;\nclass Cash {}\n" },
  ...
  const expectEdges = [
    { fromFile: "src/Order/Handler.php", line: 3, node: "Charge" },
    { fromFile: "src/Order/Handler.php", line: 3, node: "Refund" },
  ];
