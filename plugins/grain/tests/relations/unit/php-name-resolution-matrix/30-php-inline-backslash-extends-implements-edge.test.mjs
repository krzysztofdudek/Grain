// Reference-catalogue case `php-inline-backslash-extends-implements-edge` (Yggdrasil reference/relations/php/php-inline-backslash-extends-implements-edge.md), driven end-to-end
...
//
// Rule: Leading-backslash names in `extends` / `implements` clauses are absolute class /
// interface references (Rule 1), shadow-free and PSR-4-resolved. `class C extends
// \App\Base\Base implements \App\Flow\Flowable` emits both FQNs, each its own per-type edge to
// its own file.
...
test("php-inline-backslash-extends-implements-edge", async () => {
  const files = [
    { path: "src/Base/Base.php", language: "php", code: "<?php\nnamespace App\\Base;\nclass Base {}\n" },
    { path: "src/Flow/Flowable.php", language: "php", code: "<?php\nnamespace App\\Flow;\ninterface Flowable {}\n" },
  ...
  const expectEdges = [
    { fromFile: "src/Order/Handler.php", line: 3, node: "Base" },
    { fromFile: "src/Order/Handler.php", line: 3, node: "Flow" },
  ];
