// Reference-catalogue case `php-new-relative-silence` (Yggdrasil reference/relations/php/php-new-relative-silence.md), driven end-to-end
...
//
// Rule: `new Foo()` with no leading backslash is a namespace-RELATIVE class reference: in
// `namespace App` it resolves to `App\Foo` by current-namespace prepend (Rule 6, no global
// fallback). Binding it would require reconstructing the file's namespace + `use` aliases,
// which a source-only import-only tool does not do — so it is a deliberate tolerated recall
// miss (silence), never a guess at the global `\Metrics\Timer`.
...
test("php-new-relative-silence", async () => {
  const files = [
