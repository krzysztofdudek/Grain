// Reference-catalogue case `php-multi-clause-use-edge` (Yggdrasil reference/relations/php/php-multi-clause-use-edge.md), driven end-to-end
...
//
// Rule: A single `use` statement may carry comma-separated clauses, each its own import with
// its own optional `as` alias. `use App\A\Alpha as X, App\B\Beta as Y;` is two imports: edges
// `App\A\Alpha` and `App\B\Beta`, with the aliases `X`/`Y` as local bindings only. Stopping at
// the first clause loses the second; emitting an alias is a false positive.
...
test("php-multi-clause-use-edge", async () => {
  const files = [
    { path: "src/A/Alpha.php", language: "php", code: "<?php\nnamespace App\\A;\nclass Alpha {}\n" },
    { path: "src/B/Beta.php", language: "php", code: "<?php\nnamespace App\\B;\nclass Beta {}\n" },
  ...
  const expectEdges = [
    { fromFile: "src/Order/Handler.php", line: 3, node: "A" },
    { fromFile: "src/Order/Handler.php", line: 3, node: "B" },
  ];
