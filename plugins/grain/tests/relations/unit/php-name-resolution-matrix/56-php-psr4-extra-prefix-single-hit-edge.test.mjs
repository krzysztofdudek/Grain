// Reference-catalogue case `php-psr4-extra-prefix-single-hit-edge` (Yggdrasil reference/relations/php/php-psr4-extra-prefix-single-hit-edge.md), driven end-to-end
...
//
// Rule: A second, unrelated PSR-4 prefix coexisting with `App\` does not manufacture a second
// hit for the chosen prefix. With `App\` → `src/` and `Legacy\` → `legacy/`, the FQN
// `App\Pay\G` matches only the longest unique prefix `App\`, and only its root holds the file →
// a single hit → resolve. (PSR-4 forbids an empty `""` prefix, so a catch-all is skipped and
// never adds a phantom hit.)
...
test("php-psr4-extra-prefix-single-hit-edge", async () => {
  const files = [
    { path: "src/Pay/G.php", language: "php", code: "<?php\nnamespace App\\Pay;\nclass G {}\n" },
    { path: "src/Order/Handler.php", language: "php", code: "<?php\nnamespace App\\Order;\nuse App\\Pay\\G;\nclass Handler {}\n" },
  ];
  const configFiles = [
    { path: "composer.json", code: "{ \"autoload\": { \"psr-4\": { \"App\\\\\": \"src/\", \"Legacy\\\\\": \"legacy/\" } } }\n" },
  ];
  const expectEdges = [
    { fromFile: "src/Order/Handler.php", line: 3, node: "Pay" },
  ];
