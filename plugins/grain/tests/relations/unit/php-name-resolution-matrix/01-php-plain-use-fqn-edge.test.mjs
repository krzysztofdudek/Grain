// Reference-catalogue case `php-plain-use-fqn-edge` (Yggdrasil reference/relations/php/php-plain-use-fqn-edge.md), driven end-to-end
...
//
// Rule: A plain `use App\Foo;` imports the fully-qualified type `App\Foo` and binds the local
// name `Foo`. The operand is absolute (import names are not processed relative to the current
// namespace), so the extractor emits the FQN verbatim; the resolver maps it to a `.php` file by
// composer PSR-4 (`App\` → `src/` → `src/Payment/Gateway.php`). The `use` line IS the per-type
// edge — a real dependency on the file declaring that exact FQN.
...
test("php-plain-use-fqn-edge", async () => {
  const files = [
    { path: "src/Payment/Gateway.php", language: "php", code: "<?php\nnamespace App\\Payment;\nclass Gateway {}\n" },
  ...
  const expectEdges = [
    { fromFile: "src/Order/Handler.php", line: 3, node: "Payment" },
  ];
