// Reference-catalogue case `php-inline-backslash-new-edge` (Yggdrasil reference/relations/php/php-inline-backslash-new-edge.md), driven end-to-end
...
//
// Rule: A leading-backslash inline reference `new \App\Metrics\Timer()` is ABSOLUTE: the `\`
// marks it as resolved from the global namespace, independent of the file's `namespace` and
// `use` aliases (Rule 1) — so it is shadow-free and maps to a file by the SAME PSR-4 rule as an
// import. In a class-autoload position (here `new`) the extractor emits the FQN with one
// leading `\` stripped → `App\Metrics\Timer`.
...
test("php-inline-backslash-new-edge", async () => {
  const files = [
    { path: "src/Metrics/Timer.php", language: "php", code: "<?php\nnamespace App\\Metrics;\nclass Timer {}\n" },
  ...
  const expectEdges = [
    { fromFile: "src/Order/Handler.php", line: 3, node: "Metrics" },
  ];
