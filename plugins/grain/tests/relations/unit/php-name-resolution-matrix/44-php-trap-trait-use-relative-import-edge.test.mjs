// Reference-catalogue case `php-trap-trait-use-relative-import-edge` (Yggdrasil reference/relations/php/php-trap-trait-use-relative-import-edge.md), driven end-to-end
...
//
// Rule: `namespace Foo\Bar; use Baz\Trait1; class C { use Trait1; use Foo\Trait2; }` — the
...
// `Foo\Bar\Foo\Trait2`, current-ns relative, never `\Foo\Trait2`) are usage-site and silent.
// The two `use` operators differ: the top-level namespace `use` is absolute, the in-class trait
// `use` is namespace-relative.
...
test("php-trap-trait-use-relative-import-edge", async () => {
  const files = [
    { path: "lib/Pkg/Trait1.php", language: "php", code: "<?php\nnamespace Baz;\ntrait Trait1 {}\n" },
    { path: "src/Bar/C.php", language: "php", code: "<?php\nnamespace Foo\\Bar;\nuse Baz\\Trait1;\nclass C { use Trait1; use Foo\\Trait2; }\n" },
  ];
  const configFiles = [
    { path: "composer.json", code: "{ \"autoload\": { \"psr-4\": { \"Baz\\\\\": \"lib/Pkg/\", \"App\\\\\": \"src/\" } } }\n" },
  ];
  const expectEdges = [
    { fromFile: "src/Bar/C.php", line: 3, node: "Pkg" },
  ];
