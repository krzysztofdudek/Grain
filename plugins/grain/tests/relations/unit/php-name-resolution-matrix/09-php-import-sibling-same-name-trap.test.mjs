// Reference-catalogue case `php-import-sibling-same-name-trap` (Yggdrasil reference/relations/php/php-import-sibling-same-name-trap.md), driven end-to-end
...
//
// Rule: When two nodes each declare a type with the SAME simple name in DIFFERENT namespaces, a
// `use` import binds ONLY the namespace it names. `use App\Http\Request;` in a file whose own
// namespace is `App\Auth` (which could itself hold an `App\Auth\Request`) is the EXACT FQN
// `App\Http\Request` — there is no current-namespace prepend on a `use` operand and no sibling
// binding. The bare usage `Request` (which would resolve to the import or to the sibling) is a
// usage-site form and is silent, so the import can never mis-bind to the sibling
// `App\Auth\Request`.
...
test("php-import-sibling-same-name-trap", async () => {
  const files = [
    { path: "src/Http/Request.php", language: "php", code: "<?php\nnamespace App\\Http;\nclass Request {}\n" },
    { path: "src/Auth/Request.php", language: "php", code: "<?php\nnamespace App\\Auth;\nclass Request {}\n" },
    { path: "src/Auth/Controller.php", language: "php", code: "<?php\nnamespace App\\Auth;\nuse App\\Http\\Request;\nclass Controller { function m(Request $r) {} }\n" },
  ];
  const configFiles = [
    { path: "composer.json", code: "{ \"autoload\": { \"psr-4\": { \"App\\\\\": \"src/\" } } }\n" },
  ];
  const expectEdges = [
    { fromFile: "src/Auth/Controller.php", line: 3, node: "Http" },
  ];
