// Reference-catalogue case `php-no-global-fallback-silence` (Yggdrasil reference/relations/php/php-no-global-fallback-silence.md), driven end-to-end
...
//
// Rule: A bare class name `Logger` with no `use` import resolves to `App\Logger` by
// current-namespace prepend (Rule 6) — PHP has NO global fallback for classes, so it is NEVER
// `\Logger` or `Psr\Log\Logger`. Emitting an edge to any other node's `Logger` would be a false
// positive. The import-only model never resolves a bare class name, so no mis-binding is
// possible.
...
test("php-no-global-fallback-silence", async () => {
  const files = [
    { path: "src/Log/Logger.php", language: "php", code: "<?php\nnamespace App\\Log;\nclass Logger {}\n" },
    { path: "src/Order/Service.php", language: "php", code: "<?php\nnamespace App;\nclass Service { function f(Logger $l) {} }\n" },
  ];
  const configFiles = [
