// Reference-catalogue case `php-trap-qualified-first-segment-clash-edge` (Yggdrasil reference/relations/php/php-trap-qualified-first-segment-clash-edge.md), driven end-to-end
...
//
// Rule: `namespace App\Http; use App\Data\Builder as DB;` — the import edge is the absolute FQN
// `App\Data\Builder`. The qualified usages `new DB\QueryBuilder()` (first segment `DB`
// import-translated → `App\Data\Builder\QueryBuilder`) and `new Http\Request()` (current-ns
// prepend → `App\Http\Http\Request`) are usage-site and silent. The import never mis-binds; the
// usages emit nothing.
...
test("php-trap-qualified-first-segment-clash-edge", async () => {
  const files = [
    { path: "src/Data/Builder.php", language: "php", code: "<?php\nnamespace App\\Data;\nclass Builder {}\n" },
    { path: "src/Http/C.php", language: "php", code: "<?php\nnamespace App\\Http;\nuse App\\Data\\Builder as DB;\nclass C {}\n" },
  ];
  const configFiles = [
    { path: "composer.json", code: "{ \"autoload\": { \"psr-4\": { \"App\\\\\": \"src/\" } } }\n" },
  ];
  const expectEdges = [
    { fromFile: "src/Http/C.php", line: 3, node: "Data" },
  ];
