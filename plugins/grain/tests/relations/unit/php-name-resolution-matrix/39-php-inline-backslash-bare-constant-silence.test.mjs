// Reference-catalogue case `php-inline-backslash-bare-constant-silence` (Yggdrasil reference/relations/php/php-inline-backslash-bare-constant-silence.md), driven end-to-end
...
//
// Rule: A leading-backslash name in BARE-CONSTANT position `\App\C\FOO` does NOT trigger class
// autoloading — PHP keeps constants in a separate namespace, never mapped to a PSR-4 class
// file. The extractor's class-autoload-position allowlist excludes the generic expression
// parent of a bare constant, so it emits nothing.
...
test("php-inline-backslash-bare-constant-silence", async () => {
  const files = [
    { path: "src/C/FOO.php", language: "php", code: "<?php\nnamespace App\\C;\nclass FOO {}\n" },
