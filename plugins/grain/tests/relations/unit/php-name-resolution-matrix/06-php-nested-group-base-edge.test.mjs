// Reference-catalogue case `php-nested-group-base-edge` (Yggdrasil reference/relations/php/php-nested-group-base-edge.md), driven end-to-end
...
//
// Rule: A group clause may itself be a multi-segment qualified name. The imported FQN is the
// leading base + `\` + the clause's full (possibly multi-segment) name. In `use
// App\Sub\{Inner\Deep, Plain};` the clauses resolve to `App\Sub\Inner\Deep` and
// `App\Sub\Plain`. Splitting a clause at the wrong boundary, or dropping the `Inner\` segment,
// mis-keys the FQN.
...
test("php-nested-group-base-edge", async () => {
  const files = [
    { path: "src/Sub/Inner/Deep.php", language: "php", code: "<?php\nnamespace App\\Sub\\Inner;\nclass Deep {}\n" },
    { path: "src/Sub/Plain.php", language: "php", code: "<?php\nnamespace App\\Sub;\nclass Plain {}\n" },
  ...
  const expectEdges = [
    { fromFile: "src/Order/Handler.php", line: 3, node: "Inner" },
    { fromFile: "src/Order/Handler.php", line: 3, node: "Sub" },
  ];
