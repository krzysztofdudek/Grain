// Reference-catalogue case `php-psr4-longest-prefix-wins-edge` (Yggdrasil reference/relations/php/php-psr4-longest-prefix-wins-edge.md), driven end-to-end
...
//
// Rule: PSR-4 prefixes nest; the LONGEST matching prefix governs the base directory. With
// `App\` → `src/` and `App\Tests\` → `tests/`, the FQN `App\Tests\Unit\GatewayTest` maps under
// `App\Tests\` → `tests/Unit/GatewayTest.php`, NOT under `App\` → `src/Tests/Unit/...`. Using
// the shorter prefix would map to the wrong directory and miss the file.
...
test("php-psr4-longest-prefix-wins-edge", async () => {
  const files = [
    { path: "tests/Unit/GatewayTest.php", language: "php", code: "<?php\nnamespace App\\Tests\\Unit;\nclass GatewayTest {}\n" },
    { path: "src/Order/Handler.php", language: "php", code: "<?php\nnamespace App\\Order;\nuse App\\Tests\\Unit\\GatewayTest;\nclass Handler {}\n" },
  ];
  const configFiles = [
    { path: "composer.json", code: "{ \"autoload\": { \"psr-4\": { \"App\\\\\": \"src/\", \"App\\\\Tests\\\\\": \"tests/\" } } }\n" },
  ];
  const expectEdges = [
    { fromFile: "src/Order/Handler.php", line: 3, node: "Unit" },
  ];
