// Reference-catalogue case `php-inline-backslash-multi-catch-edge` (Yggdrasil reference/relations/php/php-inline-backslash-multi-catch-edge.md), driven end-to-end
...
//
// Rule: Leading-backslash names in a multi-catch `catch (\App\E1\DomainError |
// \App\E2\OtherError $e)` are absolute class references (Rule 1). Each caught type is wrapped
// in a named_type in a class-autoload position, shadow-free, so the extractor emits both FQNs —
// one edge each.
...
test("php-inline-backslash-multi-catch-edge", async () => {
  const files = [
    { path: "src/E1/DomainError.php", language: "php", code: "<?php\nnamespace App\\E1;\nclass DomainError {}\n" },
    { path: "src/E2/OtherError.php", language: "php", code: "<?php\nnamespace App\\E2;\nclass OtherError {}\n" },
  ...
  const expectEdges = [
    { fromFile: "src/Order/Handler.php", line: 3, node: "E1" },
    { fromFile: "src/Order/Handler.php", line: 3, node: "E2" },
  ];
