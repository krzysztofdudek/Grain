...
test('excluding the typePath candidate falls through to the live parentTypePath candidate AT THE SAME ROOT — not silence, and not a farther root', () => {
  const isExcluded = (p) => p === typePath;
  expect(javaResolve.resolveJavaFqn('com.foo.Outer.Inner', FROM, nestedDeps(isExcluded))).toBe(parentTypePath);
});
