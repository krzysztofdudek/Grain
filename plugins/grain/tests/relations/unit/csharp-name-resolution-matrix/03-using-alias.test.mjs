...
test("csharp-using-alias", async () => {
  await runCase({
    files: [
    { path: "src/c/Use.cs", language: "csharp", code: `using Gw = Foo.Bar.IGateway;
namespace App;
class C { void M() { var x = new Gw(); } }
` },
    { path: "src/g/IGateway.cs", language: "csharp", code: `namespace Foo.Bar;
public class IGateway { }
` },
  ],
    edges: [{ file: "src/c/Use.cs", line: 3, node: "g" }],
    silence: false,
  });
});
