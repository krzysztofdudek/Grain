...
test("csharp-alias-anytype-embedded", async () => {
  await runCase({
    files: [
    { path: "src/c/Use.cs", language: "csharp", code: `using Pair = (int Id, Mod.Customer Cust);
using Arr = Mod.Order[];
class C { Pair p; Arr a; }
` },
    { path: "src/m/Customer.cs", language: "csharp", code: `namespace Mod;
public class Customer { }
` },
    { path: "src/o/Order.cs", language: "csharp", code: `namespace Mod;
public class Order { }
` },
  ],
    edges: [{ file: "src/c/Use.cs", line: 1, node: "m" }, { file: "src/c/Use.cs", line: 2, node: "o" }],
    silence: false,
  });
});
