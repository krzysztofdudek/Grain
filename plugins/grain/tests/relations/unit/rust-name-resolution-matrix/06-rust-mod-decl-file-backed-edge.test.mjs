...
// grain adaptation: ported from Yggdrasil's reference-case-runner.ts `runCase('rust-mod-decl-file-
// backed-edge')`, driven by reference/relations/rust/rust-mod-decl-file-backed-edge.md. Grain has
// no reference-catalogue file or owner-index module, so the case's ## Files / ## Expect are
// embedded verbatim below and the edge-detection pipeline (SymbolTable + real resolver +
...
const files = [
  { path: 'src/orders/foo/mod.rs', language: 'rust', code: 'pub struct X;\n' },
  { path: 'src/orders/mod.rs', language: 'rust', code: 'mod foo;\n' },
];
const configFiles = [
  { path: 'Cargo.toml', code: '[package]\nname = "mycrate"\n' },
];
const expectEdges = [{ fromFile: 'src/orders/mod.rs', line: 1, node: 'foo' }];
const expectSilence = false;

test('rust-mod-decl-file-backed-edge', async () => {
