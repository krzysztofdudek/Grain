...
// grain adaptation: ported from Yggdrasil's reference-case-runner.ts `runCase('rust-pub-use-
// reexport-edge')`, driven by reference/relations/rust/rust-pub-use-reexport-edge.md. Grain has no
// reference-catalogue file or owner-index module, so the case's ## Files / ## Expect are embedded
// verbatim below and the edge-detection pipeline (SymbolTable + real resolver +
...
const files = [
  { path: 'src/api/mod.rs', language: 'rust', code: 'pub struct Handler;\n' },
  { path: 'src/c/lib.rs', language: 'rust', code: 'pub use crate::api::Handler;\n' },
];
const configFiles = [
  { path: 'Cargo.toml', code: '[package]\nname = "mycrate"\n' },
];
const expectEdges = [{ fromFile: 'src/c/lib.rs', line: 1, node: 'api' }];
const expectSilence = false;

test('rust-pub-use-reexport-edge', async () => {
