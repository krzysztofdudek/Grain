...
// grain adaptation: ported from Yggdrasil's reference-case-runner.ts `runCase('rust-inline-crate-
// expression-position-edge')`, driven by reference/relations/rust/rust-inline-crate-expression-
// position-edge.md. Grain has no reference-catalogue file or owner-index module, so the case's
// ## Files / ## Expect are embedded verbatim below and the edge-detection pipeline (SymbolTable +
// real resolver + resolveCandidateGroup; a node is the basename of a file's parent directory,
// matching the catalogue's node:<id> convention) is reimplemented over the exported unit-harness
// primitives, scoped to Rust (no C# global-using tier needed).

const files = [
  { path: 'src/logging/mod.rs', language: 'rust', code: 'pub fn init() {}\n' },
  { path: 'src/c/run.rs', language: 'rust', code: 'fn g() {\n    crate::logging::init();\n}\n' },
];
const configFiles = [
  { path: 'Cargo.toml', code: '[package]\nname = "mycrate"\n' },
];
const expectEdges = [{ fromFile: 'src/c/run.rs', line: 2, node: 'logging' }];
const expectSilence = false;

test('rust-inline-crate-expression-position-edge', async () => {
