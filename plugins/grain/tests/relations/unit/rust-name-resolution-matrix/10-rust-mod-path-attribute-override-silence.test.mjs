...
// grain adaptation: ported from Yggdrasil's reference-case-runner.ts `runCase('rust-mod-path-
// attribute-override-silence')`, driven by reference/relations/rust/rust-mod-path-attribute-
// override-silence.md. Grain has no reference-catalogue file or owner-index module, so the case's
...
const files = [
  { path: 'src/c/custom/loc.rs', language: 'rust', code: 'pub struct X;\n' },
  { path: 'src/c/lib.rs', language: 'rust', code: '#[path = "custom/loc.rs"]\nmod foo;\n' },
];
const configFiles = [
...
test('rust-mod-path-attribute-override-silence', async () => {
