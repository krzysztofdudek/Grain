...
// Ported from Yggdrasil's typescript-name-resolution-matrix.test.ts case 'typescript-import-attribute-json-silence',
// backed by reference/relations/typescript/typescript-import-attribute-json-silence.md. This file inlines the
...
test('typescript-import-attribute-json-silence', async () => {
  // grain adaptation: r/data/a.json has no grain grammar (no JSON extractor in the source either) — materialized to disk for the resolver's existence-probing but excluded from parsing/extraction.

  const files = [
  { path: "r/app/use.ts", language: "typescript", code: "import data from '../data/a.json' with { type: 'json' };\nconsole.log(data);\n" },
  ];
  const materializeOnlyFiles = [
  { path: "r/data/a.json", code: "{ \"a\": 1 }\n" },
  ];
  const expectEdges = [
