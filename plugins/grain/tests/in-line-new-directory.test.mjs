// the locator line must not assert a place that does not exist.
//
// `check <file> --as <a path in a directory that does not exist yet>` is the ONE call an author makes
// before writing the first file of a new directory — the case this guard was written for (trial-0.4.0 §4b:
...
// of a place. Same disease class as the never-parsed note's "this concept isn't in the repository" and the zero-foothold banner's
...
test('a path inside an EXISTING module still reads exactly as before', () => {
...
test('a path whose module has NO files never claims a layer or a fan-in for it', () => {
...
test('the hedged locator names the nearest ancestor that does exist, with ITS real numbers', () => {
...
test('a repo with no module graph still returns null, unchanged', () => {
