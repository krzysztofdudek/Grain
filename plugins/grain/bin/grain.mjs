...
if (!process.execArgv.includes(FLAG) && process.env.GRAIN_V8 !== 'off' && process.argv[2] !== 'refresh') {
  const r = spawnSync(process.execPath, [FLAG, fileURLToPath(import.meta.url), ...process.argv.slice(2)], { stdio: 'inherit', env: { ...process.env, GRAIN_V8: 'off' } });
...
catch (e) { console.error('[grain] ' + (process.env.GRAIN_DEBUG ? (e.stack || e) : (e?.message || e))); code = 1; }
