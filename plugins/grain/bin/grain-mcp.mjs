...
class ProtocolError extends Error { constructor(code, message) { super(message); this.code = code; } }
...
async function buildCtx(repoArg, args, opts) {
  ...
  const { model, meta, head, banner, stale } = await ensureFresh({ root, isGit, store, opts: {}, want: 'refresh' });
...
const optionalRepoProp = { repo: { type: 'string', description: "Absolute path to the repository root. Defaults to this MCP server's own working directory." } };

// ----- the curated tool set: read-only pass-throughs over already-tested CLI commands, nothing new -----
const TOOLS = {
  grain_where: {
    description: 'Ask this repository where something like <query> lives, what convention governs that place, and which exemplar to copy — call this BEFORE creating a new source file, or whenever it is unclear where new code belongs. Use the repository\'s own vocabulary (a decorator, a base type, a file or function name), not a paraphrase of it.',
    inputSchema: {
      type: 'object',
      properties: { query: { type: 'string', description: 'Intent words describing what is about to be built, in the repo\'s own vocabulary (e.g. "http handler for orders", "database migration").' }, ...optionalRepoProp },
  ...
  grain_status: {
    ...
    validate() {},
    async exec(a) { const ctx = await buildCtx(a.repo, [], { json: true }); const [json] = await cmdStatus(ctx); return json; },
  },
  grain_report: {
    description: 'Get this repository\'s top established conventions with evidence and trends, grouped by partition — a broad survey, not a query about one place or file.',
    inputSchema: {
      type: 'object',
      properties: { top: { type: 'number', description: 'How many top conventions to include per partition (default 15).' }, ...optionalRepoProp },
      additionalProperties: false,
    },
    validate(a) { if (a?.top !== undefined && typeof a.top !== 'number') throw invalidParams('grain_report: "top" must be a number'); },
    async exec(a) { const ctx = await buildCtx(a.repo, [], { json: true, top: a.top }); const [json] = await cmdReport(ctx); return json; },
  },
};
...
async function handleRequest(id, method, params) {
  ...
  if (method === 'tools/list') {
    result(id, { tools: Object.entries(TOOLS).map(([name, t]) => ({ name, description: t.description, inputSchema: t.inputSchema })) });
    return;
  }
  if (method === 'tools/call') {
    const name = params?.name; const args = params?.arguments || {};
    const tool = TOOLS[name];
    if (!tool) { errorResponse(id, -32602, `Unknown tool: ${name}`); return; }
    try { tool.validate(args); }
    catch (e) { if (e instanceof ProtocolError) { errorResponse(id, e.code, e.message); return; } throw e; }
    try { const text = await tool.exec(args); result(id, { content: [{ type: 'text', text }], isError: false }); }
    catch (e) { result(id, { content: [{ type: 'text', text: e?.message || String(e) }], isError: true }); } // an execution failure (bad repo, unknown file, …) — never a protocol error, never a crash
    return;
  }
...
async function handleLine(line) {
  ...
  let msg; try { msg = JSON.parse(trimmed); } catch { send({ jsonrpc: '2.0', id: null, error: { code: -32700, message: 'Parse error' } }); return; }
  const hasId = msg && Object.prototype.hasOwnProperty.call(msg, 'id');
  if (!msg || typeof msg.method !== 'string') { if (hasId) errorResponse(msg?.id ?? null, -32600, 'Invalid Request'); return; }
  if (!hasId) return; // a notification (initialized, cancelled, …) — nothing this server needs to act on, and never a response
  try { await handleRequest(msg.id, msg.method, msg.params); }
  catch (e) { errorResponse(msg.id, -32603, e?.message || String(e)); } // internal error — still never crash the process
}
...
rl.on('line', line => { queue = queue.then(() => handleLine(line)).catch(e => console.error('[grain-mcp]', e?.stack || e)); });
rl.on('close', () => { queue.finally(() => process.stdout.write('', () => process.exit(0))); });
