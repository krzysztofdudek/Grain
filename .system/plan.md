...
**Bramka go/no-go na całą sekcję: I2b.** Jeśli plaster z przykładu nie bije grepa na własnej historii prywatnego repozytorium
...
**Kryterium.** Na prywatnym repozytorium (N=100): pokrycie plastra ≥ pokrycie grepa przy precyzji ≥ 2× grepa — inaczej
...
2. **I2 → I2b** — plaster z przykładu i bramka na prywatnym repozytorium. **Tu decyzja: dalej czy stop.**
...
(H.fps ≤ 20k — O(n) przy każdym `how`) — wybrać wariant bez bumpa, jeśli pomiar na prywatnym repozytorium < 100 ms.
...
**Kryterium (zapisane tu, żeby nie ruszać go po fakcie):** na prywatnym repozytorium i na ≥ 6 repozytoriach korpusu z
...
2. Recenzent Opus (`gate-J2.3`): uruchamia `selftest --how --last 100` na prywatnym repozytorium (ścieżka od utrzymującego;
