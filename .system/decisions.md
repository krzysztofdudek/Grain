...
## 2026-09-05 · north-star-brownfield-miner · class G
USER RULING (2026-09-05): funkcja celu Graina zmienia się. Konsument to MAINTAINER wdrażający Yggdrasila na obcym (brownfield) repo, nie agent przy edycji. Gwiazda polarna: Grain skraca drogę od git clone obcego repo do działającego .yggdrasil/ (architektura, węzły, aspekty w drafcie, plan refaktoru) i zmusza, żeby graf powstał z dowodów w kodzie i historii, nie z wyobraźni. Metryka: na repo z istniejącym ręcznym grafem (Yggdrasil jako wzorzec nr 1, examples/* jako smoke; prywatne wdrożenia użytkownika niedostępne w tej sesji) — recall: jaki ułamek typów/when, mapowań, relacji, granic Grain proponuje sam; precision: jaki ułamek propozycji maintainer przyjmuje. Yggdrasil pozostaje TYLKO referencją — nic tam nie zmieniamy. Zastępuje §0 system.md (stara gwiazda: 'odpowiada na więcej pytań niż grep') i unieważnia próby sparowane na diffach agenta jako miarkę. Prace nad adopcją hooków/where dla agenta wstrzymane, nie odrzucone.

## 2026-09-05 · sub-gate-rows-are-the-product · class G
LEKCJA z uruchomienia Graina na Yggdrasilu (2026-09-05, 93 s, 19 partycji, 273 grupy, 182 konwencje, 37 modułów, 2 cykle identyczne z yg advise, 2 archNorms równe ręcznym deny): partycje z kompresji pokrywają się ~1:1 z when.path typów w yg-architecture.yaml; grupy ról (declarations(11): ParsedFile→DeclaredSymbol, node.childForFieldName) to zalążki aspektów. Ale prawdziwe reguły domowe (cli-command-contract: catch→abortOnUnexpectedError) leżą POD bramką λ jako obs share 0.78 — praktykowane w 22% miejsc. Dla agenta przy edycji odmowa certyfikacji jest uczciwa; dla maintainera wiersz pod bramką to plan refaktoru. Konsekwencja: λ=8 zostaje dla powierzchni agentowych, ale powierzchnia dla maintainera musi pokazywać kratę z udziałami (kandydaci na reguły + adopcja %), nie tylko certyfikowane NORM. Surowiec jest cały w grain export (partitions, moduleGraph, archNorms, groups+markers, templates, deviatingSites, twins, cycles); brakuje renderera na format .yggdrasil/ i instrumentu mierzącego rekonstrukcję.
...
## 2026-09-05 · law-loop-is-the-bet · class G
DIRECTOR RULING (2026-09-05) po memo Fable-counsel (.system/research/counsel-factory-2026-09-05.md), w ramach gwiazdy 'miner brownfieldu' i wizji użytkownika 'ekosystem, od człowieka tylko wymagania': kierunek toru 2 = PĘTLA PRAWA (B1). Praktyka wydobyta przez Graina → kandydat na regułę jako WYGENEROWANY deterministyczny check.mjs (z deskryptora check w exporcie; enumeratory: import-of, call-of, decorator, extends, returns, name-shape, placement, lexical) → korpus drills wycięty z historii z hold-outem czasowym (conformingSites→satisfies, deviatingSites→violates, tylko miejsca powstałe po sha cięcia) → replay yg simulate / yg drill → draft→advisory na kontrakcie 0 MISS / 0 FALSE-ALARM (kontrakt, nie próg) → enforced TYLKO podpisem człowieka z liczbami (klasa renewal) → retirement z aspects --health catch/exposure. Proza (content.md) tylko dla tego, co nie ma kształtu. Konsekwencje: (1) 094 renderuje check.mjs, nie prozę (steer wysłany); (2) ticket 097 = pomiar B1 na Yggdrasilu w klonie: bramki PRZED uruchomieniem — ≥10 z 20 'miner-miss' reguł odtworzonych W WERDYKCIE, precyzja ≥0.80 na próbce 20 kandydatów sklasyfikowanej przez maintainera (a/b/c); poniżej bramek 'nie robimy, z liczbami'; (3) B2 (graf odświeżany przy zamknięciu fali, reconstruct.mjs jako sensor dryfu) = ticket 098, po 094; (4) B3 (plan intencji) po B1. Reszta nieusuwalna nazwana w §5 memo: wymagania i progi w nich, reguły o nieobecności, dozwolone vs praktykowane, zarezerwowane podpisy, większość vs cnota. Nie budować wyroczni dla agenta; nie generować reguł prozą jako ścieżki głównej; nie importować strojonych progów z family-without-law.mjs; nie auto-enforce.

## 2026-09-05 · ecosystem-design-scope-split · class G
DIRECTOR (2026-09-05) po memo counsel-2 (.system/research/ecosystem-design-2026-09-05.md, scalone 413ad4c). Werdykt memo: środek pętli istnieje (Grain→architektura, Yggdrasil→prawo, Horde→organizacja, B1→drut praktyka→prawo); brakuje DWU KOŃCÓW: kompilatora intencji (wymaganie → uczestnicy, ścieżka dowodu, flow, drafty strażników, podpisany próg) i drutu wyroczni zewnętrznej (ucieczka po merge → proponowany incydent → podpis człowieka → drill). Bez drugiego każda liczba pętli prawa jest samooceną. PODZIAŁ ZAKRESU: w Grainie (ta sesja, tor 2): S0 094/096, S1 097 z provenance+I10 i retirement z drills, S2 098 + sizing.json (codelength per moduł jako liczba dla reguły cięcia Horde), próba 'szablon jako check kształtu'. POZA Grainem, decyzje użytkownika (repo Yggdrasil/Horde read-only dla mnie): (a) Yggdrasil: committed deterministic event stream albo retirement z drills; advise klasa 'family' czyta proposal Graina; cięcie scripts/family-without-law.mjs (7 strojonych stałych vs λ); (b) Horde: escalate --kind incident + renderer proponowanego incydentu; kolumny charteru threshold/never/flow; node.mjs map z ratio sizing; (c) meta-prawo: 2 aspekty deterministyczne (nieoznaczony próg liczbowy w check bez signed-by; wygenerowana reguła bez provenance.json); (d) scalenie skilla dyrektora Graina w Horde (Grain jako misja Horde; instrumenty zostają); (e) kompilator intencji jako nowe narzędzie rodziny, NIE w Grainie — Grain zostaje minerem. Zakłady nazwane w memo §6 obowiązują jako zakłady, nie ustalenia.

## 2026-09-05 · too-much-is-a-ranking-not-a-flag · ticket 096 · class G
DIRECTOR RULING na eskalację 096 (2026-09-05). Pomiar: diagnostyka 'za dużo' trafia w znane odpowiedzi bez podpowiedzi — Yggdrasil src/cli/check.ts rank 4/627 i src/core/check.ts rank 1 (yg-config sam nazywa check 'recurring hot spot'), oba cykle identyczne z yg advise, 12/15 plików z attention-dump; Grain core.mjs rank 1 i grain.mjs rank 2 z 117, jedyne dwa odpalające na wszystkich 6 wymiarach; cięcie core.mjs po 28 grupach ról +184.79 bitów, wszystkie 10 wierszy responsibilities Yggdrasila z UJEMNYM zyskiem cięcia (i mówią to). Uczciwy negatyw: unia flag nad 10 wymiarami nazywa 27.4% Yggdrasila / 10.3% Graina — jako LISTA FLAG nie przechodzi standardu 018/037; per wymiar sufit 1/λ=12.5% jest wyprowadzony. ROZSTRZYGNIĘCIE: (1) produktem jest RANKING top-N po sumie nadwyżki bitów z dowodem per wiersz, nie flaga; rozkład 'na ilu wymiarach' publikowany jako rozkład, nigdy jako próg (≥3 byłoby nową strojoną stałą); (2) koszt indeksu za ~46k porównań: NIE dodajemy drugiego etapu certyfikacji teraz — ranking nie potrzebuje selekcji, a test odstępców w core.mjs też go nie płaci (spójność); wraca jako decyzja z drugą opinią Opusa dokładnie wtedy, gdy jakaś powierzchnia zechce z tego zrobić FLAGĘ lub bramkę; (3) responsibilities najsłabszy wymiar (48.7% zakresów niejednoznacznych nie należy do żadnej grupy) — ujawniać, nie łatać; (4) leave-one-out jako znaczenie 'pointwise' przyjęte; minRaw=5 wyprowadzone jako punkt osiągalności bramki, nie druga podłoga. Wejście do renderera 094/098 jako backlog refaktoru: top-N + zysk cięcia.

## 2026-09-05 · premerge-only-from-main-checkout · class D
LEKCJA (dyrektor, 2026-09-05): premerge.mjs uruchomiony z WNĘTRZA worktree gałęzi wykrywa root jako ten worktree i wpisuje nowe testy do drzewa, w którym implementacja już istnieje — revert-test pokazuje '0 fail / 11 tests' i wygląda jak brak dowodu czerwono-zielonego, choć to artefakt. Regułą: premerge zawsze z głównego checkoutu (/home/user/Grain), z --base <gałąź sesji>. Do dopisania w narzędziu: odmowa, gdy root jest worktree (.git jest plikiem).

## 2026-09-05 · instrument-floors-allowed-if-stated-and-measured · ticket 094 · class G
DIRECTOR RULING (2026-09-05, 094): stałe dopuszczeniowe W INSTRUMENTACH (tests/stress/*.mjs) — MIN_TYPE_FILES=2, MIN_PROMOTE_FILES, MIN_CONVENTION_SITES, SUBGATE_PER_PARTITION — są dopuszczalne, jeśli (1) są nazwane w raporcie jako podłogi, (2) ich koszt jest zmierzony (094 zmierzyło tylko MIN_TYPE_FILES: layered-architecture 0/3 vs 3/3 w 093), (3) nie wchodzą do silnika. Warunek na przejście do powierzchni produktu (komenda grain propose): każda podłoga zmierzona wrażliwością na Yggdrasilu i Grainie albo wyprowadzona; inaczej eskalacja z liczbami. Zasada 'zero strojonych progów' dotyczy silnika i jego mowy; instrument może mieć podłogę, ale nie może jej ukrywać.

## 2026-09-05 · no-rewrite-one-chain-three-gates · class G
DIRECTOR (2026-09-05) po memo counsel-3 (.system/research/counsel-rewrite-and-factory-2026-09-05.md, scalone 2a5cec8). Przyjęte jako kierunek w zakresie Graina, reszta to decyzje użytkownika: (1) NIE przepisywać Yggdrasila — wszystkie 9 zmian po stronie Yggdrasila z memo 1 i 2 są addytywne i na brzegu (<1% drzewa 69k linii); jedna 'walczy z kształtem' (committed deterministic event stream) i ma alternatywę (retirement z drills); test akceptacji przepisania to remis z 8100 testami i golden hasha; changelog v5: 264 fixów vs 202 dodań, 39% wpisów o mówieniu prawdy — to materiał, którego rewrite nie ma. (2) Ranking 096 = stały backlog utrzymaniowy Yggdrasila (dwa cykle po najsłabszych krawędziach: relations→core 2 zależności, portal→cli 4; top-10 nadwyżki; bliźniak core::11~core::4; podwójna referencja komend; 12 reguł bez drilli). Pomiar decydujący: re-run 096 po pierwszej fali stranglera — czy top rows się ruszyły i czy yg advise nadal nominuje cykl. (3) Fabryka = (γ): JEDEN łańcuch wymaganie → flow → reguła z provenance → werdykt adresowany hashem → podpisany incydent → drill; DWIE epistemologie (λ Graina, kontrakty+podpisy Yggdrasila); TRZY bramki (repo osobno). Nie monorepo (Grain padłby na aspektach Yggdrasila w dniu 1; trzy kadencje i uczciwe statusy kolidują). Kontrakty szwów do wersjonowania: .grain/proposal/, provenance.json, measurements/<R>.json, kolumny charteru, incidents.md; test szwu przez realny binary sąsiada (wzór: staged yg check z 094). (4) 'Factory' na żadnym README dopóki ledger incydentów nie ma wpisu podpisanego przez człowieka — to product claim, pozycja 3 listy eskalacji Horde, idzie do użytkownika. (5) Charter fabryki pisalny do tabeli dowodów; trzy wiersze bez liczby (E6 latencja ucieczka→drill, E7 koszt/ticket, E8 wierność kompilatora), dwa z pożyczonym 0.80 (E1, E8), jeden z liczbą dyrektora do potwierdzenia (E2 = 097) — progi są użytkownika; jedna kalibracyjna misja Horde (kolejka Graina pod Horde, S3) daje bazę E7. (6) Do użytkownika: klasyfikacja a/b/c 59 proponowanych typów bez odpowiednika i próbki 20 kandydatów z 097 — do tego czasu każda precyzja w rodzinie jest surowym rozjazdem (jego własna reguła).

## 2026-09-05 · layers-compatible-no-user-thresholds · class G
USER RULING (2026-09-05): 'Nie będę definiował progów — sam masz na czym sprawdzić. Zostawmy układ: zależnie od potrzeby instaluję Yggdrasil, Graina albo/i Hordę czy inne; cała rodzina ma być kompatybilna i współpracująca — warstwy.' Konsekwencje: (1) PROGI NIE SĄ WEJŚCIEM OD CZŁOWIEKA. Każda bramka akceptacji (E1 recall/precyzja propozycji, E2 bramki 097, E8 wierność) jest WYPROWADZANA z wzorca (ręczny graf Yggdrasila, examples, Grain jako brownfield nr 2) i instrumentów, zapisana przed przebiegiem, ujawniona z metodą — nigdy pytana. Memo counsel-2/3 wymagające 'podpisu maintainera pod progiem' czytać jako: próg ma pochodzenie (skąd liczba), nie podpis. (2) KLASYFIKACJA ROZJAZDÓW a/b/c nie jest zadaniem użytkownika: robi ją niezależny sędzia Opus (nie autor instrumentu) na dowodach z kodu i historii, z ujawnieniem, że to sędzia-model; ruling oracle-is-fallible pozostaje co do symetrii raportu. (3) KSZTAŁT PRODUKTU = WARSTWY: osobno instalowalne narzędzia (Yggdrasil = prawo, Grain = miner, Horde = organizacja, Ratatoskr/Urd/Researcher = intencja/dyscyplina/pomiar), każde z własnym repo i bramką, KOMPATYBILNE przez wersjonowane kontrakty szwów (.grain/proposal/ z provenance.json; format .family-candidates.json dla yg advise; sizing.json dla Horde; measurements/<R>.json; incidents.md) i testowane przez realny binary sąsiada (wzór: staged yg check z 094). Nie monorepo. To jest (γ)+(β) z counsel-3 bez podpisów pod progami. (4) Wymagania pozostają jedynym wejściem od człowieka; 'factory' to rodzina działająca razem, nie nowe repo.

## 2026-09-05 · granularity-bounded-by-evidence-not-taste · class G
USER RULING (2026-09-05): 'Im granularniejsze reguły, tym lepiej — przyszłe sesje agenta nie mają wyjścia, więc im bardziej docinasz architekturę i aspekty, tym lepiej ostatecznie.' Konsekwencje: (1) Renderer PREFERUJE NAJDROBNIEJSZE cięcie, którego każdy element niesie własny dowód (share, n, miejsca) i którego wyrenderowany check przechodzi drill z 0 FALSE-ALARM — granicę granularności wyznacza dowód, nie gust ani wysiłek recenzenta (recenzenta nie ma; certyfikuje drill). Alternatywy z 094 (284 drobniejsze typy) stają się AKTYWNE, gdy mają dowód; węzeł jednoplikowy dopuszczalny, gdy plik jest własną lokalnością (entry-point, vendor). MIN_TYPE_FILES=2 do zmierzenia jako podłoga do usunięcia, nie do obrony. (2) PRZEWAŻENIE METRYKI: precyzja względem ręcznego grafu przestaje być celem — ręczny graf jest grubszy niż to, co użytkownik chce; zostaje jako sanity (recall: nic, co ręka zapisała, nie ginie). Nowa precyzja = udział elementów propozycji, które (a) Yggdrasil ładuje, (b) produkują pary z werdyktem, (c) mają drill, który łapie (violates-* MISS=0) i nie fałszuje (FA=0). To jest 'na ile Grain sensownie gada vs na czym operuje Yggdrasil'. (3) Rozjazd 'Grain drobniej niż ręka' (klasa c z 093) liczy się na korzyść Graina, jeśli spełnia (a)-(c). (4) Ryzyko nazwane: eksplozja reguł bez łapań — strażnikiem pozostaje retirement z drilli i I2/I9 obok liczby reguł; reguła bez historycznego łapania zostaje draft.

## 2026-09-05 · two-granularities-rules-fine-nodes-ownership-sized · class G
DIRECTOR (2026-09-05), ocena koncepcyjna 'jak Horde radzi sobie na grafie' (node.mjs, roster.mjs, brief.mjs, model.md): Horde czyta z .yggdrasil/model/**/yg-node.yaml tylko name/type/description/mapping/relations (własny wąski parser YAML; aspects rozpoznane, ale nie zapisywane; yg-architecture.yaml nieczytany — sprawdzenie when: przy mapowaniu jest ręcznym krokiem dyrektora); charter.md i contracts.md leżą obok yg-node.yaml, ale Yggdrasil ich nie weryfikuje; kontrakt Horde (test/scenariusz w contracts.md, zatwierdzany przez architekta) NIE jest mapowany na port/aspekt/flow Yggdrasila, więc zatwierdzony kontrakt nie jest egzekwowany; hierarchia Horde to zespoły w rosterze, nie zagnieżdżenie węzłów grafu; niezmiennik 'ticket nazywa jeden węzeł, dwa gdy niesie kontrakt' i 'właściciel per dotknięty węzeł' skalują się z liczbą węzłów. WNIOSEK: ruling granularity-bounded-by-evidence-not-taste dotyczy DWÓCH RÓŻNYCH GRANULARNOŚCI i trzeba je rozdzielić: (1) REGUŁY I TYPY — jak najdrobniej z dowodem: typy z when (path+content), aspekty scope.per: file, coverage.type_level, żeby reguła wiązała się z plikiem przez typ bez żadnego węzła; to jest to, czego przyszłe sesje agenta 'nie mają wyjścia' spełnić; (2) WĘZŁY — rozmiar własnościowy pod Horde: charter+kontrakty+kod mieszczą się w jednym kontekście Sonneta (liczba z sizing.json z 098), ticket nazywa jeden węzeł, właściciel per węzeł nie eksploduje. Drobne węzły nie są potrzebne do drobnych reguł — Yggdrasil już to rozdziela (type_level, per: file). Konsekwencja dla renderera 094: 'alternatywy → aktywne' stosuje się do TYPÓW i ASPEKTÓW, cięcie WĘZŁÓW zostaje grube (72 na Yggdrasilu) i dostaje sizing. Po stronie Graina (do 100/098): propozycja emituje też charter.md per węzeł z karty where (lives in / depends on / used by / konwencje / egzemplarze / współzmiany) — brief właściciela Horde z dowodów, nie z głowy; sizing.json jako liczba dla reguły cięcia. Po stronie Horde (użytkownik): (a) kontrakt zatwierdzony → drukować stanzę portu/aspektu yg (kontrakt egzekwowany, nie tylko zapisany); (b) czytać aspects: i yg-architecture.yaml (typy, when, dozwolone relacje) zamiast ręcznego kroku; (c) własny parser YAML zastąpić zwalidowanym podzbiorem (reconstruct.mjs: 830/830 zgodnych z yaml) — yg-node.yaml nie jest generowany przez yg, piszą go ludzie i agenci, więc kształt nie jest 'wąski i przewidywalny'; (d) hierarchia zespołów z zagnieżdżenia węzłów, nie z rostera. Najmocniejszy drut już działa: worker uruchamia yg check --approve --only-deterministic jako pierwszą akcję, więc relation-conformance i mapowania przechodzą przez architekta.

## 2026-09-05 · law-loop-b1-not-doing-it-with-numbers · ticket 097 · class G
DIRECTOR RULING (2026-09-05, 097, .system/research/law-loop-yggdrasil.md): B1 'pętla prawa' jako GŁÓWNY drut praktyka→prawo NIE WCHODZI — z liczbami. Bramka 1 zapisana przed przebiegiem (≥10/20 reguł 'miner-miss' odtworzonych w werdykcie) dała 2/20, a przy wymogu wspólnych plików 0/20; zakład 'szablon superpozycji jako check kształtu' przegrany (1/20, FA 690→1701). Diagnoza, nie strojenie: reguły maintainera są negatywne i specyficzne, bo ZAPADŁA DECYZJA ('nigdy node:fs tutaj'), a Grain widzi większości; dwie się pokryły przypadkiem. To rozszerza granicę z 093 (6 reguł o nieobecności) na: nawet gdy identyfikator jest w kodzie, wyrenderowana reguła ląduje gdzie indziej. Co ZOSTAJE (zmierzone): (1) maszyneria hold-outu po przodku commita, I10 = 21/21, provenance.json na każdej regule — obowiązkowa dla każdej przyszłej generacji reguł; (2) errs: under to własność reguły WEWNĄTRZ jej zakresu (na obcych korpusach 24/49 checków fałszuje) — tak czytać 094; (3) held-out sweep 156/25/1 na 182 przypadkach: szablony czytają drzewo tak, jak Grain liczył, na kodzie, którego miner nie widział — wierność szablonu, nie jakość reguły. NOWY KSZTAŁT PRODUKTU REGUŁ: Grain NIE generuje prawa; Grain dostarcza (a) architekturę (typy/węzły/relacje/cykle/sizing/dług — zmierzone 093/094/098), (b) styl leksykalny i kształt nazw jako reguły deterministyczne z drillem (jedyne kandydaty w próbce 097, które są regułami w sensie maintainera), (c) kratę pod bramką jako KANDYDATÓW NA DECYZJE z dowodem (udział, n, miejsca) — decyzja o 'nigdy' pozostaje po stronie wymagań (ruling layers: człowiek daje wymagania; kolumna 'never'). Retirement z aspects --health: sygnał istnieje po jednym lokalnym fillu, ale lock deterministyczny jest gitignored — między maszynami nadal nie; ticket po stronie Yggdrasila pozostaje, przeformułowany na committed stream. yg simulate jest non-comparable na brownfieldzie z konstrukcji — replay dla propozycji musi być Grain-side. Bramka 2 (próbka 20): klasyfikuje sędzia Opus w 101, nie użytkownik.

## 2026-09-05 · prose-aspects-draft-by-default · ticket 101 · class D
Aspekty prozowe (content.md) w propozycji Graina są domyślnie DRAFT i oznaczone jako wyłączone z bramki: 101 zmierzył, że 1305/1671 propozycji prozowych na 17 repo korpusu jest bezwładne bez klucza (keyless CI ich nie egzekwuje, drill ich nie testuje), a sędzia Opus ocenił wskaźnik sensu prozy na 0%. Grain dostarcza jako egzekwowalne tylko to, co ma check.mjs albo relację w grafie; proza zostaje jako kandydat do decyzji człowieka, nigdy jako reguła z enforce: strict. Zmiana wchodzi do renderera 094 w następnej iteracji — do tego czasu sekcja 'The proposal contract' w docs/reference.md musi to mówić wprost.

## 2026-09-05 · drill-fa-labelling-is-acceptance-not-defect · ticket 101 · class D
Wada z §8.1 raportu 101 (check.mjs o zasięgu plikowym wyprowadzony z konwencji pod-plikowej daje fałszywe alarmy w drillu na plikach mieszanych) NIE jest naprawiana jako defekt: to decyzja odbiorcza o etykietowaniu drilla — Yggdrasil liczy zasięg aspektu per plik, Grain zna zasięg per symbol, a przemapowanie w dół zawsze coś traci. Rozwiązanie: renderer 094 ma oznaczać takie reguły jako 'file-scope approximation' w provenance.json i obniżać je do DRAFT, gdy drill na własnym korpusie repo daje >0 fałszywych alarmów. Próg 0 nie jest gustem — to jedyna wartość, przy której keyless CI nie zablokuje uczciwej zmiany.

## 2026-09-05 · root-fix-accepted-min-type-files-goes · ticket 101 · class D
Poprawka renderowania węzła _root (nie jest ścieżką ani gitlinkiem) przyjęta — podniosła wskaźnik sensu aspektów deterministycznych z 57% do 60% na korpusie 17 repo. MIN_TYPE_FILES z propose.mjs jest usuwany: 101 wykazał, że nie jest nośny (zmiana wartości nie zmienia ani liczby ładowalnych grafów 17/17, ani wskaźnika sensu); parametr bez wpływu na wynik to ukryty próg, a progów nie definiujemy — wyprowadzamy z danych.
...
## 2026-09-05 · enforced-requires-certified-origin · ticket 107 · class D
Status 'enforced' w propozycji wymaga DWÓCH rzeczy: pochodzenia z certyfikowanej konwencji (udział nad własną bramką certyfikacji Graina, MDL/λ) ORAZ realnego drilla z 0 fałszywych alarmów i ≥1 złapanym mutantem. Wiersz z kraty pod bramką (origin sub-gate-lattice), który przeszedł drill, dostaje 'advisory' — u Yggdrasil to 'reviewer runs; refused → warning (no block)', zapisywany w baseline — i to on jest 'kandydatem' w cichym raporcie. Powód z liczb: grain propose na samym Grainie (0 certyfikowanych konwencji, repo młode) wypromował 22/22 wierszy kraty do enforced, w tym reguły praktykowane w 67% miejsc z 22 z 44 odstępstwami, których własny nagłówek mówi 'BELOW grain's certification bound (0.875)'. Egzekwowanie reguły, którą Grain sam odmówił certyfikować, łamie orzeczenie sub-gate-rows-are-the-product (krata = plan refaktoru, nie prawo) i zrobiłoby yg check czerwonym na jednej trzeciej istniejącego kodu. Drill mierzy poprawność check.mjs, nie adopcję reguły — dlatego sam nie wystarcza.
...
## 2026-09-05 · precision-holds-recall-tracks-imports · ticket 108 · class G
Cztery niezależne ręczne wyrocznie (Yggdrasil TS, express CJS, Grain JS, spring-petclinic Java): precyzja relacji Graina 0.929–1.000 na wszystkich, także na express bez żadnej bramki importów w CI — 0.998 na Yggdrasil NIE było sufitem od CI. Recall śledzi wyłącznie to, ile grafu zależności jest zapisane jako rozwiązywalny import: 0.894 TS → 0.114 Java (strukturalne: source roots i głębokość cięcia modułów, bilet 113). Z 1335 relacji Grain dodał 3, których żadna wyrocznia nie ma, w tym celowo niezadeklarowane naruszenie w express. Konsekwencja: twierdzenia Graina o zależnościach można egzekwować od razu (precyzja), a ich kompletność zależy od języka i musi być raportowana per repo jako pokrycie, nie jako stała.

## 2026-09-06 · oracle-is-a-living-graph · ticket 117 · class D
Ręczna wyrocznia Graina (tests/stress/oracles/grain) jest żywym grafem Graina, nie zamrożonym fixture'em: refaktor przenoszący pliki aktualizuje ją jak każdy maintainer (dług grafu klasy b z orzeczenia oracle-is-fallible), z datowaną notatką w README, żeby każdy wcześniejszy pomiar pozostał przypisany do swojego sha. Przy 117 jeden węzeł z 29 ścieżkami nie przeszedł (type-when-mismatch, prompt-too-large, cykl fasady) — poprawny kształt to cztery węzły wielkości własności (fasada, mining, zapytania, montaż modelu) i predykat typu jako dopełnienie katalogu engine/, żeby jutrzejszy moduł był sklasyfikowany w dniu, w którym powstanie. Parametr reviewer.tiers.standard.max_prompt_chars=600000 był ustawiony przez plik, którego już nie ma — zostaje do decyzji przy następnej zmianie wyroczni (dziś prawdziwe byłoby ~300000).
...
## 2026-09-06 · esc-23 · ticket 131
APPROVED as a structural fix, not a tuning: the 5000-pair cap on model.scopeCochange is split into two populations with the same total budget (within-file and cross-file capped separately, each sorted by descending support), so a repository with large files no longer drops every cross-file pair before any consumer sees them. Ticket 146. Condition: before merge, re-measure the existing co-change consumers (completeness, where's partners, what's tested-by) on the four oracles with the same instruments as before; if any precision drops, the change does not ship. After it lands, grain advise's relation kind is re-measured with the SAME mutual gate and floor; its verdict stays 'data, not advice' until that re-measurement says otherwise.

## 2026-09-06 · esc-22 · ticket 131
NO. A separate, lower support floor for cross-file scope pairs is a new tunable constant, and the evidence standard for a pair should not depend on where the pair lives. First remove the bias 23 measured (ticket 146), then re-measure cross-file pairs under the existing floor; only a measurement after that may reopen this.
